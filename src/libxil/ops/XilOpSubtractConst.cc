...
//------------------------------------------------------------------------
//
//  File:	XilOpSubtractConst.cc
//  Project:	XIL
//  Revision:	1.9
//  Last Mod:	10:07:03, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)XilOpSubtractConst.cc	1.9\t00/03/10  "
...
class XilOpSubtractConst : public XilOpPoint {
public:
...
protected:
    XilOpSubtractConst(XilOpNumber op_num);
    virtual ~XilOpSubtractConst();
};


XilOp*
XilOpSubtractConst::create(char  function_name[],
                           void* args[],
                           int   )
{
    static XilOpCache  subtractconst_op_cache;
    XilImage*          src    = (XilImage*)args[0];
    float*             values = (float*)args[1];
    XilImage*          dst    = (XilImage*)args[2];
    
    XilOpNumber opnum;
    if((opnum = xili_verify_op_args(function_name, &subtractconst_op_cache,
                                    dst, src)) == -1) {
        return NULL;
    }
   ...
   //
    ...
    XilOpSubtractConst* op = new XilOpSubtractConst(opnum);
    if(op == NULL) {
	XIL_ERROR(dst->getSystemState(), XIL_ERROR_RESOURCE, "di-1", TRUE);
        return NULL;
    }
...
XilOpSubtractConst::XilOpSubtractConst(XilOpNumber op_num) : XilOpPoint(op_num) { }
XilOpSubtractConst::~XilOpSubtractConst() { }

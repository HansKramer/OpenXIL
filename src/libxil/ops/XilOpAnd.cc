...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)XilOpAnd.cc	1.7\t00/03/10  "
...
class XilOpAnd : public XilOpPoint {
public:
...
protected:
    XilOpAnd(XilOpNumber op_num);
    virtual ~XilOpAnd();
};


XilOp*
XilOpAnd::create(char  function_name[],
		 void* args[],
		 int)
{
    ...
    //
    static XilOpCache	and_op_cache;
    XilOpNumber		opnum =
        xili_verify_op_args(function_name, &and_op_cache, dst, src1, src2);
    if(opnum == -1) {
	return NULL;
    }
    ...
    XilOpAnd* op = new XilOpAnd(opnum);
    if(op == NULL) {
...
XilOpAnd::XilOpAnd(XilOpNumber op_num) : XilOpPoint(op_num) { }
XilOpAnd::~XilOpAnd() { }

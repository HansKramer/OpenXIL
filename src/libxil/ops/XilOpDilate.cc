...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)XilOpDilate.cc	1.14\t00/03/10  "
...
class XilOpDilate : public XilOpAreaKernel {
public:
...
protected:
    XilOpDilate(XilOpNumber op_num);
    virtual ~XilOpDilate();
};
...
//
XilOp*
XilOpDilate::create(char  function_name[],
                    void* args[],
                    int   )
{
    static XilOpCache	dilate_op_cache;
    XilImage*           src = (XilImage*)args[0];
    XilImage*           dst = (XilImage*)args[1];

    XilOpNumber		opnum;						   
    if((opnum = xili_verify_op_args(function_name, &dilate_op_cache,
                                    dst, src))== -1) {
        return NULL;
    }
    ...
    XilOpDilate* op = new XilOpDilate(opnum);
    if(op == NULL) {
...
XilOpDilate::XilOpDilate(XilOpNumber op_num) : XilOpAreaKernel(op_num) { }
XilOpDilate::~XilOpDilate() { }

...
//------------------------------------------------------------------------
//
//  File:	XilOpXorConst.cc
//  Project:	XIL
//  Revision:	1.10
//  Last Mod:	10:07:12, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)XilOpXorConst.cc	1.10\t00/03/10  "
...
class XilOpXorConst : public XilOpPoint {
public:
...
protected:
    XilOpXorConst(XilOpNumber op_num);
    virtual ~XilOpXorConst();
};


XilOp*
XilOpXorConst::create(char  function_name[],
		 void* args[],
		 int)
{
    ...
    //
    static XilOpCache	xor_const_op_cache;
    XilOpNumber 	opnum =
        xili_verify_op_args(function_name, &xor_const_op_cache, dst, src);
    if(opnum == -1) {
	return NULL;
    }
    ...
    //  Create an op
    //
    XilOpXorConst* op = new XilOpXorConst(opnum);
    if(op == NULL) {
	delete const_array;
...
XilOpXorConst::XilOpXorConst(XilOpNumber op_num) : XilOpPoint(op_num) { }
XilOpXorConst::~XilOpXorConst() { }

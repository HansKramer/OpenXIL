...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Lookup.cc	1.3\t00/03/10  "
...
XilStatus
XilDeviceIOcg6::lookup8DisplayPre(XilOp*        op,
                                  unsigned      ,
                                  XilRoi*       ,
                                  void**        compute_data,
                                  unsigned int* )
{
    ...
    if(lookup->getNumEntries() == 256) {
        *compute_data = (void*)-1;
        ...
        //
        *compute_data =
            (void*)deviceManager->getLookupTable(state, lookup);
    }
...
XilStatus
XilDeviceIOcg6::lookup8DisplayPost(XilOp* ,
                                   void*  compute_data)
{
    deviceManager->releaseLookupTable((int)compute_data);

    return XIL_SUCCESS;
}

XilStatus
XilDeviceIOcg6::lookup8Display(XilOp*       op,
                               unsigned int ,
                               XilRoi*      roi,
                               XilBoxList*  bl)
{
    ...
    if(table == -1) {
        ...
        return lookup8to8(op, roi, bl,
                          (Xil_unsigned8*)lookup->getData());
    } else {
        return lookup8to8(op, roi, bl,
                          deviceManager->refLookupCache(table));
    }
}

XilStatus
XilDeviceIOcg6::lookup8to8(XilOp*         op,
                           XilRoi*        roi,
                           XilBoxList*    bl,
                           Xil_unsigned8* lut)
{

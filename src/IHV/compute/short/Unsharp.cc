        ...
        if (size == 5) {
            ...
            while (kh--) {
                for (j = 0; j < src1_xsize; j++) {
                    fsum  = (float)(*src);  src += src1_pixel_stride;
                    fsum += (float)(*src);  src += src1_pixel_stride;
                    fsum += (float)(*src);  src += src1_pixel_stride;
                    fsum += (float)(*src);  src += src1_pixel_stride;
                    fsum += (float)(*src);
            ...
            for (i = 0; i < src1_xsize; i++) {
                fsum  = *(result + i);  result += src1_xsize;
                fsum += *(result + i);  result += src1_xsize;
                fsum += *(result + i);  result += src1_xsize;
                fsum += *(result + i);  result += src1_xsize;
                fsum += *(result + i);

                *dst = _XILI_ROUND_S16(fsum*norm);
            ...
            for (i = 0; i < src1_ysize-1; i++) {
                ...
                //  vertical
                roll0 -= 1;   if (roll0 < 0) roll0 = size-1;
                roll1 -= 1;   if (roll1 < 0) roll1 = size-1;
                roll2 -= 1;   if (roll2 < 0) roll2 = size-1;
                roll3 -= 1;   if (roll3 < 0) roll3 = size-1;
                roll4 -= 1;   if (roll4 < 0) roll4 = size-1;

                for (j = 0; j < src1_xsize; j++) {
                    fsum  = *(result + j);  result += src1_xsize;
                    fsum += *(result + j);  result += src1_xsize;
                    fsum += *(result + j);  result += src1_xsize;
                    fsum += *(result + j);  result += src1_xsize;
                    fsum += *(result + j);
...
XilStatus
XilDeviceManagerComputeSHORT::Unsharp(XilOp*       op,
                                     unsigned     op_count,
                                     XilRoi*      roi,
                                     XilBoxList*  bl)
{

...
typedef struct {
    ...
    Xil_signed16	*src_pixel;	// curr pixel in image
...
} sfill16_stack_frame_t; 

struct sfill16_stack_block {
    struct sfill16_stack_block	*prev;
    struct sfill16_stack_block	*next;
    sfill16_stack_frame_t	frame[XILI_FILL_MAX_STACK_FRAMES];
};
typedef struct sfill16_stack_block sfill16_stack_block_t;

struct sfill16_stack_ctrl {
    struct sfill16_stack_block	*curr_block;
    int				curr_idx;
};
typedef struct sfill16_stack_ctrl sfill16_stack_ctrl_t;
...
typedef struct {
    ...
    struct sfill16_stack_ctrl	*stack_ctrl;
} sfill16_data_t;
...
//
static int
SoftFill8(long x, long y, Xil_signed16* src_pixel,
	  Xil_unsigned8* fill_scan, float* weight_pixel,
	  sfill16_data_t* fill_ptr, XilSystemState* err_state);

static int
Sfill16Push(sfill16_stack_ctrl_t *ctrl_ptr, 
	   sfill16_stack_frame_t *local_frame_ptr);

static int
Sfill16Pop(sfill16_stack_ctrl_t *ctrl_ptr, 
	  sfill16_stack_frame_t *local_frame_ptr);

static Xil_boolean
Sfill16PixelInside(Xil_signed16* pixel,
		  float* weight_pixel,
		  Xil_signed16* bg,
		  unsigned int nbands,
		  float* mtx,
...
static void
Sfill16Shadow(sfill16_stack_frame_t *frame,
	     sfill16_data_t	 *fill_ptr,
	     long		 lx,
	     Xil_signed16	 *pixel,
	     float	 	 *weight_pixel,
	     long		 rx);
...
static Xil_boolean
Sfill16PixelInsideG(XilStorage* storage,
		   float* weight_pixel,
		   Xil_signed16* bg,
		   unsigned int nbands,
		   float* mtx,
...
//
XilStatus
XilDeviceManagerComputeSHORT::SoftFill(XilOp*       op,
				      unsigned,
				      XilRoi*      roi,
				      XilBoxList*  bl)
{
    ...
    // Get bg colors
    Xil_signed16* bgcolor;
    op->getParam(5, (void**)&bgcolor);

    // Get fill color
    Xil_signed16* fill_color;
    ...
    float* color_diff = new float[nbands];
    if(color_diff == NULL) {
    ...
    Xil_signed16* last_bgcolor = &bgcolor[nbands*(num_bg-1)];
	...
	//
	   ...
	   (dest_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
	    ...
	    //
	    Xil_signed16* src_pixel = src1_data +
	    ...
	    //
	    sfill16_stack_ctrl_t *ctrl = new sfill16_stack_ctrl_t;
	    if(ctrl == NULL) {
		delete [] weight_base_addr;
		delete [] fill_base_addr;
		delete [] mtx_invert;
	    ...
	    sfill16_stack_block_t *block = new sfill16_stack_block_t;
	    if(block == NULL) {
		delete ctrl;
		delete [] weight_base_addr;
		delete [] fill_base_addr;
		...
		return XIL_FAILURE;
            ...
            while(rl.getNext(&x, &y, &xsize, &ysize)) {
                Xil_signed16* dst_scanline = dest_data +
		...
		for (; ysize > 0; ysize--) {
			...
			if (BFILL_MASK_ON(fill_scanline,
				...
				float tmp_band = (float)(*src_band) -
				    (*weight_pixel * color_diff[k]);
				*dst_band = _XILI_ROUND_S16(tmp_band);
				src_band++;
				dst_band++;
			    }
			}
		...
		return XIL_FAILURE;
			...
			for(unsigned int xx = xsize; xx > 0; xx--) {
				...
				// color with new fill color.  New pixel =
				// pixel-(weight*fgcolor)+(weight*fill_color)
				float tmp_band = (float)(*src_pixel) -
				    (*weight_pixel * color_diff[band]);
				*dst_pixel = _XILI_ROUND_S16(tmp_band);
			    }
...
SoftFill8(long x, long y, Xil_signed16* src_pixel,
	  ...
	  sfill16_data_t* fill_ptr, XilSystemState* err_state)
{
    sfill16_stack_frame_t  curr_frame;
    long  lx;
    long rx;
    Xil_signed16	*left_src_pixel;
	...
	while ( (lx >= 0) && Sfill16PixelInside(left_src_pixel,
					       left_weight_pixel,
					       fill_ptr->bg,
					       fill_ptr->nbands,
					       fill_ptr->mtx,
	...
	while ((rx <= fill_ptr->xmax) && Sfill16PixelInside(src_pixel,
							   weight_pixel,
							   fill_ptr->bg,
							   fill_ptr->nbands,
							   fill_ptr->mtx,
    ...
    if (Sfill16Push(fill_ptr->stack_ctrl,&curr_frame)==XIL_FAILURE) {
	return(XIL_FAILURE);
    }
	...
	if (Sfill16Pop(fill_ptr->stack_ctrl,&curr_frame)==XIL_FAILURE) {
	   return(XIL_FAILURE);
	}
	...
	if ((y <= fill_ptr->ymax) && (y >= 0)) {
	    ...
	    inside = Sfill16PixelInside(src_pixel, weight_pixel,
		...
		while ((x >= 0) && Sfill16PixelInside(src_pixel,
						     weight_pixel,
						     fill_ptr->bg,
						     fill_ptr->nbands,
						     fill_ptr->mtx,
		...
		if (inside) {
		    if (Sfill16PixelInside(src_pixel, weight_pixel,
					  fill_ptr->bg,
					  fill_ptr->nbands,
					  fill_ptr->mtx, fill_ptr->nbg,
					  fill_scan, x)) {
			...
			// no longer inside
			Sfill16Shadow(&curr_frame, fill_ptr, lx,
		...
		Sfill16Shadow(&curr_frame, fill_ptr, lx, left_src_pixel,
...
{
    sfill16_stack_frame_t	new_frame;
    long			newy;
    Xil_signed16		*new_pixel;
    ...
    Sfill16Push(fill_ptr->stack_ctrl, &new_frame);
	   ...
	   int box_x, int box_y)
{
	...
	left_weight_pixel = weight_pixel;
        ...
        // check seed pixel within region
	while ( (lx >= 0) && Sfill16PixelInsideG(src_storage,
						left_weight_pixel,
						fill_ptr->bg,
						fill_ptr->nbands,
						fill_ptr->mtx,
	...
	while ((rx <= fill_ptr->xmax) && Sfill16PixelInsideG(src_storage,
							    weight_pixel,
							    fill_ptr->bg,
							    fill_ptr->nbands,
							    fill_ptr->mtx,
	...
	if ((y <= fill_ptr->ymax) && (y >= 0)) {
	    ...
	    // here to check left
	    weight_pixel = curr_frame.weight_pixel;
	    inside = Sfill16PixelInsideG(src_storage, weight_pixel,
					fill_ptr->bg, fill_ptr->nbands,
					fill_ptr->mtx, fill_ptr->nbg,
		...
		while ((x >= 0) && Sfill16PixelInsideG(src_storage,
						      weight_pixel,
						      fill_ptr->bg,
						      fill_ptr->nbands,
						      fill_ptr->mtx,
		...
		if (inside) {
		    if (Sfill16PixelInsideG(src_storage, weight_pixel,
					   fill_ptr->bg,
					   fill_ptr->nbands,
					   fill_ptr->mtx, fill_ptr->nbg,
					   fill_scan, x, y, box_x, box_y)) {
	    ...
	    if (inside) {
		// hit the edge of region while still inside scan
		Sfill16Shadow(&curr_frame, fill_ptr, lx, NULL,
			     left_weight_pixel, (x-1));
	    }
	}    // end if within y bounds
    }    // end while, done with fill loop, stack empty
		   ...
		   int box_y)
{
	...
	for (unsigned int b = 0; b < nbands; b++) {
	    ...
	    storage->getStorageInfo(b,
				    ...
				    NULL,
				    (void**) &data);
	    Xil_signed16* pixel = data + (y - box_y) * scanline_stride +

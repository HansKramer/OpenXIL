...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Threshold.cc	1.7\t00/03/10  "

#include "XilDeviceManagerComputeSHORT.hh"
#include "ComputeInfo.hh"

XilStatus
XilDeviceManagerComputeSHORT::Threshold(XilOp*       op,
                                        unsigned     op_count,
                                        XilRoi*      roi,
                                        XilBoxList*  bl)
{
    ...
    while(ci.hasMoreInfo()) {
        Xil_signed16* low  = op_low;
        Xil_signed16* high = op_high;
        Xil_signed16* map  = op_map;
...
XilStatus
XilDeviceManagerComputeSHORT::ThresholdThreshold(XilOp*       op,
                                                 unsigned     op_count,
                                                 XilRoi*      roi,
                                                 XilBoxList*  bl)
{
    ...
    //  First Threshold
    //
    Xil_signed16* op1_low;
    op1->getParam(1, (void**)&op1_low);

    Xil_signed16* op1_high;
    op1->getParam(2, (void**)&op1_high);

    Xil_signed16* op1_map;
    op1->getParam(3, (void**)&op1_map);

    //
    //  Second Threshold
    //
    Xil_signed16* op2_low;
    op2->getParam(1, (void**)&op2_low);

    Xil_signed16* op2_high;
    op2->getParam(2, (void**)&op2_high);

    Xil_signed16* op2_map;
    op2->getParam(3, (void**)&op2_map);

    while(ci.hasMoreInfo()) {
        Xil_signed16* low1  = op1_low;
        Xil_signed16* high1 = op1_high;
        Xil_signed16* map1  = op1_map;

        Xil_signed16* low2  = op2_low;
        Xil_signed16* high2 = op2_high;
        Xil_signed16* map2  = op2_map;

	COMPUTE_GENERAL_1S_1D_W_BAND(Xil_signed16, Xil_signed16,

                                     Xil_signed16 val = *src1;
                                     if(val > *high1) {
                                         if(val > *high2) {
                                             *dest = val;
                                         } else if(val < *low2) {
...
XilStatus
XilDeviceManagerComputeSHORT::Threshold_1BAND(XilOp*       op,
                                             unsigned     op_count,
                                             XilRoi*      roi,
                                             XilBoxList*  bl)
{
    if(op->getDstImage(1)->getNumBands() != 1 ||
       sizeof(Xil_signed16) != 2) {
        return XIL_FAILURE;
    }
    ...
    while(ci.hasMoreInfo()) {
        if(ci.destScanlineStride == ci.xsize &&
           ci.src1ScanlineStride == ci.xsize) {
            ...
            Xil_signed16* src_data  = ci.src1Data;
            Xil_signed16* dst_data  = ci.destData;
            Xil_signed16* src       = src_data;
            Xil_signed16* stop      = src_data + (ci.xsize*ci.ysize);
            ...
            while(src < stop) {
                if(status == 0) {
                    ...
                    int bytes = (current_i - status_i)<<1;
                    if(bytes < _XIL_FUNC_THRESH) {
                        Xil_signed16* s = src_data+status_i;
                        Xil_signed16* d = dst_data+status_i;

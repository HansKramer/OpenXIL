...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)SubtractFromConst.cc	1.8\t00/03/10  "

#include "XilDeviceManagerComputeSHORT.hh"
#include "ComputeInfo.hh"

XilStatus
XilDeviceManagerComputeSHORT::SubtractFromConst(XilOp*       op,
                                                unsigned     op_count,
                                                XilRoi*      roi,
                                                XilBoxList*  bl)
{
    ...
    while(ci.hasMoreInfo()) {
        ...
        COMPUTE_GENERAL_1S_1D_W_BAND(Xil_signed16, Xil_signed16,
                                     ...
                                     *dest = _XILI_CLAMP_S16(result),
                                     ...
                                     result = *(consts+1) - *(src1+1);
                                     *(dest+1) = _XILI_CLAMP_S16(result);
                                     result = *(consts+2) - *(src1+2);
                                     *(dest+2) = _XILI_CLAMP_S16(result),
                                      
                                     result = *(consts+band) - *src1;
                                     *dest  = _XILI_CLAMP_S16(result)
            );
    }

...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpGeneral.cc	1.19\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::TablewarpGeneral(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned int i=dst_xstart; i<dst_xend;  i++) {
                ...
                do {
                    XILI_INTERP_GENERAL_SHORT(pixel_addr,
                                              src_scanline_stride,
                                              src_pixel_stride,
                                              v_ptr,
                                              h_ptr,
    ...
    for(unsigned int b=0; b<nbands; b++) {
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(float j=dst_ystart; j<dst_yend; j++) {
                ...
                for(float i=dst_xstart; i<dst_xend;  i++) {
                    //
                    //  Calculate the source coordinates by reading the
                    //  displacement from the warp table and then moving the
                    //  values into source box space.

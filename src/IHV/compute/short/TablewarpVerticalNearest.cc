...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpVerticalNearest.cc	1.15\t00/03/10  "

#include "XilDeviceManagerComputeSHORT.hh"
#include "XiliUtils.hh"
#include "xili_geom_utils.hh"

static XilStatus tablewarp_v_nearest(XilBoxList*   bl,
                                   TablewarpData td);
...
XilStatus
XilDeviceManagerComputeSHORT::TablewarpVerticalNearest(XilOp*       op,
                                                      unsigned     ,
                                                      XilRoi*      roi,
                                                      XilBoxList*  bl)
{
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                ...
                int isy = j + xili_tablewarp_offset_int(*warp_pixel) +
                    src_to_box_y;
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(unsigned j=dst_ystart; j<dst_yend; j++) {
                ...
                for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                    ...
                    //
                    int isx = i + src_to_box_x;
                    int isy = j + xili_tablewarp_offset_int(*warp_pixel_b0) +
                        src_to_box_y;

...
struct bfill16_stack_block {
    bfill16_stack_block* prev;
    bfill16_stack_block* next;
    bfill16_stack_frame  frame[XILI_FILL_MAX_STACK_FRAMES];
};

struct bfill16_stack_ctrl {
    bfill16_stack_block* curr_block;
    int                 curr_idx;
};
...
struct bfill16_data {
    ...
    Xil_signed16*      bound;            // boundary pixel array
    unsigned int       nbands;
    ...
    bfill16_stack_ctrl* stack_ctrl;       // points to stack ctrl block
};
...
//
static int BoundaryFill16(int            x,
                          int            y,
                          Xil_signed16*  src_pixel,
                          Xil_unsigned8* fill_scan,
                          bfill16_data*  fill_ptr);

static int BFill16Push(bfill16_stack_ctrl*  ctrl_ptr,
                       bfill16_stack_frame* local_frame_ptr);

static int BFill16Pop(bfill16_stack_ctrl*  ctrl_ptr,
                      bfill16_stack_frame* local_frame_ptr);

inline Xil_boolean BFill16PixelBoundary(Xil_signed16*  pixel,
                                        Xil_signed16*  bound,
                                        unsigned int   nbands);

static void BFill16Shadow(bfill16_stack_frame* frame,
                          bfill16_data*        fill_ptr,
                          int                  lx,
                          Xil_signed16*        pixel,
                          int                  rx);

static int  BoundaryFill16G(int            x,
                            int            y,
...
static Xil_boolean BFill16PixelBoundaryG(XilStorage*    storage,
                                         unsigned int   x,
                                         unsigned int   y,
                                         Xil_signed16*  bound,
                                         unsigned int   nbands,
...
//
XilStatus
XilDeviceManagerComputeSHORT::Fill(XilOp*       op,
                                   unsigned     ,
                                   XilRoi*      roi,
                                   XilBoxList*  bl)
{
    ...
    while(bl->getNext(&src_box, &dst_box)) {
        ...
        if((src_storage.isType(XIL_PIXEL_SEQUENTIAL)) &&
           (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
            ...
            //
            Xil_signed16*  src_pixel = src_data +
            ...
            //
            bfill16_stack_ctrl* ctrl = new bfill16_stack_ctrl;
            if(ctrl == NULL) {
                delete [] fill_base_addr;
                XIL_ERROR(err_state, XIL_ERROR_RESOURCE, "di-1", TRUE);
                return XIL_FAILURE;
            }
            bfill16_stack_block* block = new bfill16_stack_block;
            if(block == NULL) {
                delete ctrl;
                delete [] fill_base_addr;
                XIL_ERROR(err_state, XIL_ERROR_RESOURCE, "di-1", TRUE);
            ...
            //  coordinates here.
            //
            if(BoundaryFill16(x_seed, y_seed, src_pixel, fill_scanline,
            ...
            //
            //  NOTE:  The src_pixel in bfill16_stack_frame and related slots
            //         in bfill16_data are not used for this case.
            //
            ...
            //
            if(BoundaryFill16G(x_seed, y_seed, &src_storage, fill_scanline,
                               &fill_data,
...
inline
Xil_boolean
BFill16PixelBoundary(Xil_signed16*  pixel,
                     Xil_signed16*  bound,
                     unsigned int   nbands)
{
...
static
int
BoundaryFill16(int            x,
               int            y,
               Xil_signed16*  src_pixel,
               Xil_unsigned8* fill_scan,
               bfill16_data*   fill_ptr)
{
    bfill16_stack_frame curr_frame;
    int                lx;
    int                rx;
    ...
    if((y <= fill_ptr->ymax) && (y >= 0)) {
        ...
        //
        while((lx >= 0) && !(BFill16PixelBoundary(left_src_pixel,
                                                 fill_ptr->bound,
                                                 fill_ptr->nbands)) ) {
            //
            //  Set pixel mask
        ...
        while((rx <= fill_ptr->xmax) &&
              !(BFill16PixelBoundary(src_pixel,
                                    fill_ptr->bound,
                                    fill_ptr->nbands))) {
            //
            //  Set pixel mask
    ...
    if(BFill16Push(fill_ptr->stack_ctrl, &curr_frame) == XIL_FAILURE) {
        return XIL_FAILURE;
    }
    ...
    while(!(BFILL_STACK_EMPTY(fill_ptr->stack_ctrl))) {
        ...
        //
        if(BFill16Pop(fill_ptr->stack_ctrl, &curr_frame) == XIL_FAILURE) {
           return XIL_FAILURE;
        }
        ...
        if((y <= fill_ptr->ymax) && (y >= 0)) {
            ...
            while(x <= fill_ptr->xmax) {
                ...
                if(inside) {
                    if(!(BFill16PixelBoundary(src_pixel,
                                             fill_ptr->bound,
                                             fill_ptr->nbands)) &&
                       (!(BFILL_MASK_ON(fill_scan, x)))) {
                        //
                        ...
                        //  No longer inside
                        //
                        BFill16Shadow(&curr_frame, fill_ptr,
            ...
            if(inside) {
                ...
                //
                BFill16Shadow(&curr_frame, fill_ptr, lx, left_src_pixel, (x-1));
            }
        }
    }
...
static
int
BFill16Push(bfill16_stack_ctrl*  ctrl_ptr, 
            bfill16_stack_frame* local_frame_ptr)
{
    int        idx;
...
static
int
BFill16Pop(bfill16_stack_ctrl*  ctrl_ptr, 
           bfill16_stack_frame* local_frame_ptr)
{
    int        idx;
...
static
void
BFill16Shadow(bfill16_stack_frame* frame,
              bfill16_data*        fill_ptr,
              int                 lx,
              Xil_signed16*       pixel,
              int                 rx)
    
{
    ...
    BFill16Push(fill_ptr->stack_ctrl, &new_frame);
                ...
                int            box_y)
{
    ...
    if((y <= fill_ptr->ymax) && (y >= 0)) {
        ...
        //  while INSIDE go left
        //
        while((lx >= 0) && !(BFill16PixelBoundaryG(src_storage, lx, y,
                                                  fill_ptr->bound,
                                                  fill_ptr->nbands,
                                                  box_x, box_y)) ) {
            //
        ...
        //
        while((rx <= fill_ptr->xmax) &&
              !(BFill16PixelBoundaryG(src_storage, rx, y,
                                     fill_ptr->bound,
                                     fill_ptr->nbands,
                                     box_x, box_y))) {
            //
    ...
    while(!(BFILL_STACK_EMPTY(fill_ptr->stack_ctrl))) {
        ...
        if((y <= fill_ptr->ymax) && (y >= 0)) {
            ...
            //
            inside =
                (!(BFill16PixelBoundaryG(src_storage, x, y,
                                        fill_ptr->bound,
                                        fill_ptr->nbands,
                                        box_x, box_y))) &&
                (!(BFILL_MASK_ON(fill_scan, x)));
            ...
            while(x <= fill_ptr->xmax) {
                ...
                if(inside) {
                    if(!(BFill16PixelBoundaryG(src_storage, x, y,
                                              fill_ptr->bound,
                                              fill_ptr->nbands,
                                              box_x, box_y)) &&
                       (!(BFILL_MASK_ON(fill_scan, x)))) {
                        ...
                        //  No longer inside
                        //
                        BFill16Shadow(&curr_frame, fill_ptr, lx, NULL, (x-1));
                        inside = FALSE;
                    }
                } else {
                    if(x > curr_frame.rx) {
...
static
Xil_boolean
BFill16PixelBoundaryG(XilStorage*    storage,
                      unsigned int   x,
                      unsigned int   y,
                      Xil_signed16*  bound,
                      unsigned int   nbands,

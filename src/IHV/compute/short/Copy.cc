...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Copy.cc	1.10\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::Copy(XilOp*      op,
                                   unsigned    ,
                                   XilRoi*     roi,
                                   XilBoxList* bl)
{
	   ...
	   (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
                ...
                if((nbands == src_pixel_stride) &&
                   (nbands == dst_pixel_stride)) {
                    unsigned int scan_size =
                        xsize * nbands * sizeof(Xil_signed16);

                    //
                    //  Can we block copy?
                    //
                    if((scan_size == src_scanline_stride*sizeof(Xil_signed16))
                       &&
                       (scan_size == dst_scanline_stride*sizeof(Xil_signed16))) {
                        //
            ...
            while(rl.getNext(&x, &y, &xsize, &ysize)) {
                ...
                for(unsigned int band=0; band<nbands; band++) {
                    ...
                    unsigned int scan_size = xsize * sizeof(Xil_signed16);

...
void
ycc709_to_rgblinear(Xil_signed16 *i_ptr,
                    Xil_signed16 *o_ptr,
                    unsigned int count,
...
{
    //
    // Color-convert YCbCr709 to RGBlinear
    //

    for (unsigned int i = 0; i < count; i++) {
      float y6, cb6, cr6, r6, g6, b6, r7, g7, b7;

      _XILI_NORMALIZE_YCC709_S(*(i_ptr), *(i_ptr+1), *(i_ptr+2),
                               &y6, &cb6, &cr6);

      _XILI_YCC709_TO_NL(y6, cb6, cr6, &r7, &g7, &b7);
...
void
ycc709_to_rgb709(Xil_signed16 *i_ptr,
                 Xil_signed16 *o_ptr,
                 unsigned int count,
                 unsigned int src_pixel_stride,
                 unsigned int dst_pixel_stride)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      _XILI_NORMALIZE_YCC709_S(*(i_ptr), *(i_ptr+1), *(i_ptr+2), &y, &cb, &cr);
...
void
ycc709_to_photoycc(Xil_signed16 *i_ptr,
                   Xil_signed16 *o_ptr,
                   unsigned int count,
                   unsigned int src_pixel_stride,
                   unsigned int dst_pixel_stride)
...
void
ycc709_to_ycc601(Xil_signed16 *i_ptr,
                 Xil_signed16 *o_ptr,
                 unsigned int count,
                 unsigned int src_pixel_stride,
                 unsigned int dst_pixel_stride)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      _XILI_QUANTIZE_YCC601_S(y601, cb601, cr601, o_ptr, o_ptr+1, o_ptr+2);
...
void
ycc709_to_cmy(Xil_signed16 *i_ptr,
              Xil_signed16 *o_ptr,
              unsigned int count,
              unsigned int src_pixel_stride,
              unsigned int dst_pixel_stride)
{
    //
    // Color-convert YCbCr709 to Y709
    //
...
void
ycc709_to_y709(Xil_signed16 *i_ptr,
              Xil_signed16 *o_ptr,
              unsigned int count,
              unsigned int src_pixel_stride,
              unsigned int dst_pixel_stride)
...
void
ycc709_to_cmyk(Xil_signed16 *i_ptr,
               Xil_signed16 *o_ptr,
               unsigned int count,
               unsigned int src_pixel_stride,
               unsigned int dst_pixel_stride)
{
    //
    // Color-convert YCbCr709 to CMYK
    //
...
void
ycc709_to_ylinear(Xil_signed16 *i_ptr,
                  Xil_signed16 *o_ptr,
                  unsigned int count,
                  unsigned int src_pixel_stride,
                  unsigned int dst_pixel_stride)
{
    //
    // Color-convert YCbCr709 to ylinear
    //
...
void
ycc709_to_y601(Xil_signed16 *i_ptr,
               Xil_signed16 *o_ptr,
               unsigned int count,
               unsigned int src_pixel_stride,
               unsigned int dst_pixel_stride)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      _XILI_NORMALIZE_YCC709_S(*(i_ptr), *(i_ptr+1), *(i_ptr+2), &y7, &cb, &cr);

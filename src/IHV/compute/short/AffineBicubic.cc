...
// COPYRIGHT
//------------------------------------------------------------------------
#pragma ident  "@(#)AffineBicubic.cc	1.21\t00/03/10  "
...
static XilStatus xili_affine_pixel_sequential_BC(AffineData affine_data);
static XilStatus xili_affine_general_storage_BC(AffineData affine_data);

XilStatus
XilDeviceManagerComputeSHORT::AffineBicubic(XilOp*       op,
                                            unsigned     ,
                                            XilRoi*      roi,
                                            XilBoxList*  bl)
{
...
XilStatus
XilDeviceManagerComputeSHORT::affineBicubic(XilBoxList* bl,
                                            AffineData  ad)
{
    XilOp*    op  = ad.op;
    ...
    while(bl->getNext(&src_box, &dst_box)) {
        ...
        if((src_storage.isType(XIL_PIXEL_SEQUENTIAL)) &&
           (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
            if(xili_affine_pixel_sequential_BC(ad) == XIL_FAILURE) {
                //
                //  Mark this box as failed and if that succeeds, continue 
...
static
XilStatus
xili_affine_pixel_sequential_BC(AffineData affine_data)
{
...
fprintf(stderr, "src x=%d, y=%d\n", isx, isy);                    
                    ...
                    //
                    XILI_INTERP_BICUBIC_SHORT(src_pixel_clip,
                                              src_sstride,
                                              src_pstride,
                                              fracx, fracy,
                                              dst_pixel);

...
XilStatus
XilDeviceManagerComputeSHORT::SubsampleAdaptive(XilOp*       op,
                                                unsigned     ,
                                                XilRoi*      roi,
                                                XilBoxList*  bl)
{
        ...
        for(unsigned int i=ysize; i!=0; i--) {
            ...
            for(unsigned int j=xsize; j!=0; j--) {
                ...
                for(unsigned int b=nbands; b!=0; b--) {
                    Xil_signed16* blk_scanline = src_band;
                    float         ftmp         = 0.0F;
                    float         blk_size     = 0.0F;

                    for(unsigned int k=src_blk_y; k!=0; k--) {
                        Xil_signed16*  blk_pixel = blk_scanline;
                    ...
                    *dst_band = _XILI_ROUND_S16(ftmp/blk_size);
        ...
        for(unsigned int b=0; b<nbands; b++) {
            ...
            for(unsigned int i=ysize; i!=0; i--) {
                ...
                for(unsigned int j=xsize; j!=0; j--) {
                    ...
                    Xil_signed16* blk_scanline = src_pixel;
                    float         ftmp         = 0.0F;
                    float         blk_size     = 0.0F;
                    ...
                    *dst_pixel = _XILI_ROUND_S16(ftmp/blk_size);

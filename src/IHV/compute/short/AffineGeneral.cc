...
// COPYRIGHT
//------------------------------------------------------------------------
#pragma ident  "@(#)AffineGeneral.cc	1.20\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::AffineGeneral(XilOp*       op,
                                            unsigned     ,
                                            XilRoi*      roi,
                                            XilBoxList*  bl)
{
...
XilStatus
XilDeviceManagerComputeSHORT::affineGeneral(XilBoxList* bl,
                                            AffineData  ad)
{
    XilOp*    op  = ad.op;
...
inline
Xil_signed16
_XILI_GENERAL_INTERP(Xil_signed16*  src_pixel,
...
{
    //
    //  Do general interpolation...its effectively a
    //  seperable convolution.  For each pixel covered by
    //  the kernel, multiply it by the value contained in
    //  the horizontal kernel and the vertical kernel.
    //
    Xil_signed16*  src_s = src_pixel;
    float          sum   = 0.0F;
    for(unsigned int ky = 0; ky < v_size; ky++) {
        Xil_signed16*  src_p = src_s;
        float          v     = v_ptr[ky];

        for(unsigned int kx = 0; kx < h_size; kx++) {
            float h = h_ptr[kx];
...
xili_affine_pixel_sequential_GL(AffineData affine_data)
{
    ...
    Xil_signed16* src_data_min = src_data -
                                 (v_key*src_sstride + h_key*src_pstride);
    Xil_signed16* src_data_max = src_data_min +
...
fprintf(stderr, "src x=%d, y=%d\n", isx, isy);                    
                    ...
                    //
                    Xil_signed16* src_pixel_clip = xili_srcbox_clip(src_pixel,
                                                       src_data_min,
                                                       src_data_max);
                    ...
                    //
                    XILI_INTERP_GENERAL_SHORT(src_pixel_clip,
                                              src_sstride,
                                              src_pstride,
                                              v_ptr, h_ptr,
                                              v_size, h_size,
...
fprintf(stderr, "src x=%d, y=%d\n", isx, isy);
                    ...
                    Xil_signed16*  src_band = xili_srcbox_clip(src_pixel,
                                                  src_data_min,
                                                  src_data_max);
                    ...
                    do {
                        ...
                        //  Do general interpolation...
                        //
                        XILI_INTERP_GENERAL_SHORT(src_band,
                                                  src_sstride,
                                                  src_pstride,
                                                  v_ptr, h_ptr,
                                                  v_size, h_size,

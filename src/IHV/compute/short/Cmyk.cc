...
//  COPYRIGHT
//------------------------------------------------------------------------
#pragma ident   "@(#)Cmyk.cc	1.3\t00/03/10  "

#include "color_convert.hh"

void
cmyk_to_rgblinear(Xil_signed16 *i_ptr,
                  Xil_signed16 *o_ptr,
                  unsigned int count,
                  ...
                  unsigned int dst_pixel_stride)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      float red, green, blue, c, m, y, k;                   
                                    
      c = _XILI_NORMALIZE_S(*(i_ptr));                     
      m = _XILI_NORMALIZE_S(*(i_ptr + 1));                     
      y = _XILI_NORMALIZE_S(*(i_ptr + 2));                     
      k = _XILI_NORMALIZE_S(*(i_ptr + 3));                     
                                    
      _XILI_CMYK_TO_L(c, m, y, k, &red, &green, &blue);                    
...
void
cmyk_to_photoycc(Xil_signed16 *i_ptr,
                 Xil_signed16 *o_ptr,
                 unsigned int count,
                 unsigned int src_pixel_stride,
                 unsigned int dst_pixel_stride)
{
    //
    // Color-convert CMYK to PhotoYCC
    //

    for (unsigned int i = 0; i < count; i++) {
      float r6, g6, b6, c, m, y, k, photoy, cb, cr, r7, g7, b7;
...
void
cmyk_to_ycc601(Xil_signed16 *i_ptr,
               Xil_signed16 *o_ptr,
               unsigned int count,
               unsigned int src_pixel_stride,
               unsigned int dst_pixel_stride)
{
    //
    // Color-convert CMYK to YCbCr601
    //

    for (unsigned int i = 0; i < count; i++) {
      float r6, g6, b6, c, m, y, k, y601, cb, cr, r7, g7, b7;
...
void
cmyk_to_ycc709(Xil_signed16 *i_ptr,
               Xil_signed16 *o_ptr,
               unsigned int count,
               unsigned int src_pixel_stride,
               unsigned int dst_pixel_stride)
{
    //
    // Color-convert CMYK to YCbCr709
    //

    for (unsigned int i = 0; i < count; i++) {
      float r6, g6, b6, c, m, y, k, y709, cb, cr, r7, g7, b7;
...
void
cmyk_to_cmy(Xil_signed16 *i_ptr,
            Xil_signed16 *o_ptr,
            unsigned int count,
            unsigned int src_pixel_stride,
            unsigned int dst_pixel_stride)
...
void
cmyk_to_y709(Xil_signed16 *i_ptr,
             Xil_signed16 *o_ptr,
             unsigned int count,
             unsigned int src_pixel_stride,
             unsigned int dst_pixel_stride)
...
void
cmyk_to_ylinear(Xil_signed16 *i_ptr,
                Xil_signed16 *o_ptr,
                unsigned int count,
                unsigned int src_pixel_stride,
                unsigned int dst_pixel_stride)
{
    ...
    for (unsigned int i = 0; i < count; i++) {
      float red, green, blue, c, m, y, k, ylinear;
...
void
cmyk_to_y601(Xil_signed16 *i_ptr,
             Xil_signed16 *o_ptr,
             unsigned int count,
             unsigned int src_pixel_stride,
             unsigned int dst_pixel_stride)
...
void
cmyk_to_rgb709(Xil_signed16 *i_ptr,
               Xil_signed16 *o_ptr,
               unsigned int count,
               unsigned int src_pixel_stride,
               unsigned int dst_pixel_stride)
{
    //
    // Color-convert CMYK to RGB709
    //

    for (unsigned int i = 0; i < count; i++) {
      float r6, g6, b6, c, m, y, k, r7, g7, b7;                   

...
//------------------------------------------------------------------------
//
//  File:	TablewarpVerticalBicubic.cc
//  Project:	XIL
//  Revision:	1.19
//  Last Mod:	10:12:26, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpVerticalBicubic.cc	1.19\t00/03/10  "
...
//
static XilStatus tablewarp_v_bicubic(XilBoxList*  bl, TablewarpData td);
static XilStatus tablewarp_v_pixel_sequential_16(TablewarpData td);
static XilStatus tablewarp_v_general_storage_16(TablewarpData td);
static XilStatus tablewarp_v_pixel_sequential_f32(TablewarpData td);
static XilStatus tablewarp_v_general_storage_f32(TablewarpData td);

XilStatus
XilDeviceManagerComputeSHORT::TablewarpVerticalBicubic(XilOp*       op,
                                                      unsigned     ,
                                                      XilRoi*      roi,
                                                      XilBoxList*  bl)
{
    ...
    status = tablewarp_v_bicubic(bl, td);
...
static
XilStatus
tablewarp_v_bicubic(XilBoxList*  bl,
		  TablewarpData td)
{
    XilOp*    op   = td.op;
        ...
        if(src_storage.isType(XIL_PIXEL_SEQUENTIAL) &&
           ...
           warp_storage.isType(XIL_PIXEL_SEQUENTIAL)) {
            if(warp->getDataType() == XIL_SHORT) {
                if(tablewarp_v_pixel_sequential_16(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                ...
                if(tablewarp_v_pixel_sequential_f32(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                    //  processing the next box.  Otherwise, return XIL_FAILURE
                    //  now. 
...
static
XilStatus
tablewarp_v_pixel_sequential_16(TablewarpData td)
{
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned int i=dst_xstart; i<dst_xend;  i++) {
                ...
                //
                float fsx = i + src_to_box_x;
                float fsy = j +
                    xili_convert_tablewarp_offset(*warp_pixel) + src_to_box_y;

                //
                //  Compute the integer and fracy portions.
                //
                ...
                do {      
                    *dst_band++ = xili_interp_cubic(src_band++,
                                                    src_scanline_stride,
                                                    fracy);
                } while (--band_count);
...
static
XilStatus
tablewarp_v_pixel_sequential_f32(TablewarpData td)
{
    XilStorage*  src_storage   = td.src_storage;
    XilStorage*  warp_storage  = td.warp_storage;
    XilStorage*  dst_storage   = td.dst_storage;
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(float j=dst_ystart; j<dst_yend; j++) {
                ...
                for(float i=dst_xstart; i<dst_xend;  i++) {
                    ...
                    *dst_pixel = xili_interp_cubic(src_pixel,
                                                   src_scanline_stride,
                                                   fracy);

...
//------------------------------------------------------------------------
//
//  File:	TablewarpNearest.cc
//  Project:	XIL
//  Revision:	1.14
//  Last Mod:	10:12:23, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpNearest.cc	1.14\t00/03/10  "

#include "XilDeviceManagerComputeSHORT.hh"
#include "XiliUtils.hh"
#include "xili_geom_utils.hh"

static XilStatus tablewarp_nearest(XilBoxList*   bl,
                                   TablewarpData td);
...
XilStatus
XilDeviceManagerComputeSHORT::TablewarpNearest(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    status = tablewarp_nearest(bl, td);
...
static XilStatus
tablewarp_nearest(XilBoxList*   bl,
		  TablewarpData td)
{
    XilOp*    op   = td.op;
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                ...
                int isx = i + xili_tablewarp_offset_int(*warp_pixel) +
                    src_to_box_x;
                int isy = j + xili_tablewarp_offset_int(*(warp_pixel+1)) +
                    src_to_box_y;
                ...
                //
                Xil_signed16*  src_band   = src_data + sytable[isy] + sxtable[isx];
            ...
            for(unsigned j=dst_ystart; j<dst_yend; j++) {
                ...
                for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                    ...
                    int isx =
                        ...
                        src_to_box_x;
                    int isy =
                        j + xili_tablewarp_offset_int(*warp_pixel_b1) +
                        src_to_box_y;

...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Paint.cc	1.7\t00/03/10  "

#include "XiliUtils.hh"
#include "XilDeviceManagerComputeSHORT.hh"

XilStatus
XilDeviceManagerComputeSHORT::Paint(XilOp*       op,
				   unsigned int   ,
				   XilRoi*      roi,
				   XilBoxList*  bl)
{
    ...
    //
    Xil_float32* s2f = getShortToFloat(dest->getSystemState());
    if(s2f == NULL) {
        return XIL_FAILURE;
    }
...
#ifdef _XIL_USE_TABLE_FLT_CNV
				float painted =
                                    ((1.0F - *brush_pixel) * s2f[*src1_band]) +
                                    (*brush_pixel * color[band_count]);
#else  // _XIL_USE_TABLE_FLT_CNV
				float painted =
                                    ((1.0F - *brush_pixel) * (float)*src1_band) +
                                    (*brush_pixel * color[band_count]);
#endif // _XIL_USE_TABLE_FLT_CNV
				*dest_band = _XILI_ROUND_S16(painted);
...
#ifdef _XIL_USE_TABLE_FLT_CNV
				float painted =
                                    ((1.0F - *brush_pixel) * s2f[*src1_pixel]) +
                                    (*brush_pixel * color[band]);
#else  // _XIL_USE_TABLE_FLT_CNV
				float painted =
                                    ((1.0F - *brush_pixel) * (float)*src1_pixel) +
                                    (*brush_pixel * color[band]);
#endif // _XIL_USE_TABLE_FLT_CNV

				*dest_pixel = _XILI_ROUND_S16(painted);

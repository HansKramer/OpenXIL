...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)ScaleBilinear.cc	1.12\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::ScaleBilinear(XilOp*       op,
			  		    unsigned     ,
					    XilRoi*      roi,
					    XilBoxList*  bl)
...
static XilStatus scale_zoom2x_pixel_sequential_BL(AffineData affine_data)
{
    ...
    if(nbands == 1 && src_nx_pixel == 1 && dst_nx_pixel == 1) {
        ...
        while(rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
            ...
            //
            Xil_signed16* dst_scanline = dst_base_addr +
                (dstR_y * dst_nx_scan) +
                (dstR_x * dst_nx_pixel);
            ...
            //
            {
                ...
                op->backwardMap(dst_box,
                                (dstR_x + (int)dstR_xsize - 1),
                                (dstR_y + (int)dstR_ysize - 1),
                                src_box,
                                &src_x,
                                &src_y);
		...
		sy -= 1.0;
            ...
            if(odd_start_line) {
                ...
                line_buffer    = new Xil_signed16[dstR_xsize];
            ...
            //
            Xil_signed16* dst_p_scanline = dst_scanline;
            Xil_signed16* dst_p_pixel    = dst_scanline;
            ...
            Xil_signed16* dst_n_scanline = dst_scanline + dst_nx_scan;
            Xil_signed16* dst_n_pixel    = dst_n_scanline;
            ...
            for(i=1; i<dstR_ysize_2; i++) {
                ...
                for(int j=0; j<dstR_xsize_4; j++) {
                    ...
                    int  interp0 =
                        (int)(*src_pixel + *(src_pixel+1))>>1;
                    int  interp1 =
                        (int)(*dst_p_pixel + *src_pixel)>>1;
                    int  interp2 =
                        (int)(*(dst_p_pixel+2) + *(src_pixel+1))>>1;
                    int  interp3 =
                        (int)(*(dst_p_pixel+1) + interp0)>>1;
                    int  interp4 =
                        (int)(*(src_pixel+1) + *(src_pixel+2))>>1;
                    int  interp5 =
                        (int)(*(dst_p_pixel+3) + interp4)>>1;
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		sy -= 1.0;
            ...
            if(odd_start_line) {
                odd_lines = !odd_lines;
                dstR_ysize_2 += odd_lines;
                line_buffer    = new Xil_signed16[dstR_xsize*3];
                dst_pixel      = line_buffer;
            }
            ...
            for(i=1; i<dstR_ysize_2; i++) {
                ...
                for(int j=0; j<dstR_xsize_4; j++) {
                    ...
                    int  interp1 =
                        (int)(*dst_p_pixel + *src_pixel)>>1;
                    int  interp2 =
                        (int)(*(dst_p_pixel+6) + *(src_pixel+3))>>1;
                    int  interp3 =
                        (int)(*(dst_p_pixel+3) + interp0)>>1;
                    int  interp4 =
                        (int)(*(src_pixel+3) + *(src_pixel+6))>>1;
                    int  interp5 =
                        (int)(*(dst_p_pixel+9) + interp4)>>1;
                    ...
                    *(dst_n_pixel+9)  = interp4;
                    ...
                    interp0 =
                        (int)(*(src_pixel+1) + *(src_pixel+4))>>1;
                    interp1 =
                        (int)(*(dst_p_pixel+1) + *(src_pixel+1))>>1;
                    interp2 =
                        (int)(*(dst_p_pixel+7) + *(src_pixel+4))>>1;
                    interp3 =
                        (int)(*(dst_p_pixel+4) + interp0)>>1;
                    interp4 =
                        (int)(*(src_pixel+4) + *(src_pixel+7))>>1;
                    interp5 =
                        (int)(*(dst_p_pixel+10) + interp4)>>1;
                    ...
                    *(dst_n_pixel+10) = interp4;
                    ...
                    interp0 =
                        (int)(*(src_pixel+2) + *(src_pixel+5))>>1;
                    interp1 =
                        (int)(*(dst_p_pixel+2) + *(src_pixel+2))>>1;
                    interp2 =
                        (int)(*(dst_p_pixel+8) + *(src_pixel+5))>>1;
                    interp3 =
                        (int)(*(dst_p_pixel+5) + interp0)>>1;
                    interp4 =
                        (int)(*(src_pixel+5) + *(src_pixel+8))>>1;
                    interp5 =
                        (int)(*(dst_p_pixel+11) + interp4)>>1;
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        while(rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
            ...
            Xil_signed16* dst_scanline = dst_base_addr +
                ...
                (dstR_x * dst_nx_pixel);
            Xil_signed16* dst_pixel = dst_scanline;
            Xil_signed16* dst_band  = dst_scanline;
		...
		sy -= 1.0;
            Xil_signed16* src_scanline = src_base_addr + 
                ...
                ((int)sx * src_nx_pixel);
            Xil_signed16* src_pixel = src_scanline;
            Xil_signed16* src_band  = src_scanline;
            ...
            if(odd_start_line) {
                ...
                line_buffer = new Xil_signed16[dstR_xsize*dst_nx_pixel];
            ...
            Xil_signed16* dst_p_scanline = dst_scanline;
            Xil_signed16* dst_p_pixel    = dst_scanline;
            Xil_signed16* dst_p_band     = dst_scanline;
            ...
            Xil_signed16* dst_n_scanline = dst_scanline + dst_nx_scan;
            Xil_signed16* dst_n_pixel    = dst_n_scanline;
            Xil_signed16* dst_n_band     = dst_n_scanline;
...
// TODO: Get rid of these (possibly) unnecessary variables
    ...
    //  1)  1 BAND -> 1 BAND (no band children)
    //  2)  3 BAND -> 3 BAND (no band children)
    //  3)  4 BAND -> 4 BAND (no band children)
    //  4)  N BAND -> N BAND (everything)
    //
    ...
    if(nbands == 1 && src_nx_pixel == 1 && dst_nx_pixel == 1) {
        ...
        while(rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
            ...
            //  The starting point in the source.
            //
            float sx = (float)dstR_x * 2.0F;
            float sy = (float)dstR_y * 2.0F;
            Xil_signed16* src_scanline = src_base_addr + 
            ...
            //  Here I set a pointer to the scanline we just computed.
            //
            Xil_signed16* src_n_scanline = src_scanline + src_nx_scan;
            Xil_signed16* src_n_pixel    = src_n_scanline;
            ...
            do {
                ...
                while(dstR_xsize_4--) {
                    ...
                    //
                    int  interp1 =
                        ((int)*src_pixel     + (int)*src_n_pixel    +
                         (int)*(src_pixel+1) + (int)*(src_n_pixel+1))>>2;

                    int  interp2 =
                        ((int)*(src_pixel+2) + (int)*(src_n_pixel+2) +
                         (int)*(src_pixel+3) + (int)*(src_n_pixel+3))>>2;

                    int  interp3 =
                        ((int)*(src_pixel+4) + (int)*(src_n_pixel+4) +
                         (int)*(src_pixel+5) + (int)*(src_n_pixel+5))>>2;

                    int  interp4 =
                        ((int)*(src_pixel+6) + (int)*(src_n_pixel+6) +
                         (int)*(src_pixel+7) + (int)*(src_n_pixel+7))>>2;
        ...
        while(rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
            ...
            do {
                ...
                while(dstR_xsize_4--) {
                    ...
                    *((int*)dst_pixel) =
                         interp1_0<<16 |
                         (interp1_1&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp1_2<<16  |
                         (interp2_0&0x0000ffff);
#else
                    *((int*)dst_pixel) =
                         interp1_1<<16 |
                         (interp1_0&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp2_0<<16  |
                         (interp1_2&0x0000ffff);
#endif
                    dst_pixel += 4;
                    ...
                    *((int*)dst_pixel) =
                         interp2_1<<16 |
                         (interp2_2&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp3_0<<16  |
                         (interp3_1&0x0000ffff);
#else
                    *((int*)dst_pixel) =
                         interp2_2<<16 |
                         (interp2_1&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp3_1<<16  |
                         (interp3_0&0x0000ffff);
#endif
                    dst_pixel += 4;
                    ...
                    *((int*)dst_pixel) =
                         interp3_2<<16 |
                         (interp4_0&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp4_1<<16 |
                         (interp4_2&0x0000ffff);
#else
                    *((int*)dst_pixel) =
                         interp4_0<<16 |
                         (interp3_2&0x0000ffff);
                    *((int*)(dst_pixel+2)) =
                         interp4_2<<16 |
                         (interp4_1&0x0000ffff);
#endif
                    dst_pixel  += 4;
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        while(rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
            ...
            Xil_signed16* src_n_scanline = src_scanline + src_nx_scan;
            Xil_signed16* src_n_pixel    = src_n_scanline;
            Xil_signed16* src_n_band     = src_n_scanline;

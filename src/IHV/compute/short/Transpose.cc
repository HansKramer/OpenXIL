...
//  Project:	XIL
//  Revision:	1.10
//  Last Mod:	10:12:05, 03/10/00
//
//  Description:
//
//
//  Transpose() retrieves short src and dst images, a fliptype and
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Transpose.cc	1.10\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::Transpose(XilOp* op,
					unsigned ,
					XilRoi*      roi,
					XilBoxList*  bl)
{
	...
	if((src_storage.isType(XIL_PIXEL_SEQUENTIAL)) &&
	   (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
	    ...
	    Xil_signed16*   src_scanline;
	    Xil_signed16*   dst_scanline;	// ptr to current scanline 
	    Xil_signed16*   src_pixel;
	    Xil_signed16*   dst_pixel;	// ptr to current pixel 
	    Xil_signed16*   src_band; 
	    Xil_signed16*   dst_band; 	// ptr to the current band 
	...
	//
	...
	} else { // images are of the XIL_GENERAL type
		...
		//  list is the area created by intersecting the ROI with
		//  the destination box.
		//
		XilRectList    rl(roi, dst_box);
		//
		...
		while (rl.getNext(&dstR_x, &dstR_y, &dstR_xsize, &dstR_ysize)) {
		    ...
		    //
		    // The rectangle in the list applies to the dst, and we 
		    ...
		    //
		    // step pixel by pixel through the dst
		    // from the src rectangle
		    //
		...
		} // end of loop over rectangles
	    } // end of loop over bands
	} // enf of if statement
    } // end of loop over src and dst boxes.
    return XIL_SUCCESS;
}

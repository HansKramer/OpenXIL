...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Dilate.cc	1.12\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::Dilate(XilOp*       op,
                                     unsigned int    ,
                                     XilRoi*      roi,
                                     XilBoxList*  bl)
{
    ...
    Xil_signed16 *mmax = new Xil_signed16[nbands];
    if (mmax == NULL) {
      delete [] xdeltas;
      delete [] ydeltas;
      XIL_ERROR(dest->getSystemState(), XIL_ERROR_RESOURCE, "di-1", TRUE);

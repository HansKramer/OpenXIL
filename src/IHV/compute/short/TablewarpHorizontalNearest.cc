...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpHorizontalNearest.cc	1.14\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeSHORT::TablewarpHorizontalNearest(XilOp*       op,
                                                        unsigned     ,
                                                        XilRoi*      roi,
                                                        XilBoxList*  bl)
{
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(unsigned j=dst_ystart; j<dst_yend; j++) {
                ...
                for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                    ...
                    //  values into source box space.
                    //
                    int isx = i + xili_tablewarp_offset_int(*warp_pixel_b0) +
                        src_to_box_x;
                    int isy = j + src_to_box_y;

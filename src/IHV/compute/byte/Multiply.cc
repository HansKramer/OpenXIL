...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Multiply.cc	1.11\t00/03/10  "

#include "XilDeviceManagerComputeBYTE.hh"
#include "ComputeInfo.hh"

XilStatus
XilDeviceManagerComputeBYTE::Multiply(XilOp*       op,
                                      unsigned     op_count,
                                      XilRoi*      roi,
                                      XilBoxList*  bl)
{
    ...
    while(ci.hasMoreInfo()) {
        ...
        COMPUTE_GENERAL_2S_1D(Xil_unsigned8, Xil_unsigned8, Xil_unsigned8,
                              ...
                              *dest  = ((result & 0xffffff00) ?
                                        (0xff) : (result & 0xFF)),

                              result    = (int)*(src1+1) * (int)*(src2+1);
                              *(dest+1) = ((result & 0xffffff00) ?
                                           (0xff):(result & 0xFF));
                              result    = (int)*(src1+2) * (int)*(src2+2);
                              *(dest+2) = ((result & 0xffffff00) ?
                                           (0xff):(result & 0xFF))
            );
#else
        COMPUTE_GENERAL_2S_1D(Xil_unsigned8, Xil_unsigned8, Xil_unsigned8,

                              result = (int)(_XILI_B2F(*src1) *
                                             _XILI_B2F(*src2));
                              *dest  = ((result & 0xffffff00) ?
                                        (0xff) : (result & 0xFF)),

                              result    = (int)(_XILI_B2F(*(src1+1)) *
                                                _XILI_B2F(*(src2+1)));
                              *(dest+1) = ((result & 0xffffff00) ?
                                           (0xff):(result & 0xFF));
                              result    = (int)(_XILI_B2F(*(src1+2)) *
                                                _XILI_B2F(*(src2+2)));
                              *(dest+2) = ((result & 0xffffff00) ?
                                           (0xff):(result & 0xFF))
            );

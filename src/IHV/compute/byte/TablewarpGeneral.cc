...
//------------------------------------------------------------------------
//
//  File:	TablewarpGeneral.cc
//  Project:	XIL
//  Revision:	1.24
//  Last Mod:	10:11:12, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpGeneral.cc	1.24\t00/03/10  "
...
//
static XilStatus tablewarp_general(XilBoxList*  bl, TablewarpData td);
static XilStatus tablewarp_pixel_sequential_16(TablewarpData td);
static XilStatus tablewarp_general_storage_16(TablewarpData td);
static XilStatus tablewarp_pixel_sequential_f32(TablewarpData td);
static XilStatus tablewarp_general_storage_f32(TablewarpData td);

XilStatus
XilDeviceManagerComputeBYTE::TablewarpGeneral(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    status = tablewarp_general(bl, td);
...
static
XilStatus
tablewarp_general(XilBoxList*  bl,
		  TablewarpData td)
{
    XilOp*    op   = td.op;
        ...
        if(src_storage.isType(XIL_PIXEL_SEQUENTIAL) &&
           ...
           warp_storage.isType(XIL_PIXEL_SEQUENTIAL)) {
            if(warp->getDataType() == XIL_SHORT) {
                if(tablewarp_pixel_sequential_16(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                ...
                if(tablewarp_pixel_sequential_f32(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                    //  processing the next box.  Otherwise, return XIL_FAILURE
                    //  now. 
            ...
            if(warp->getDataType() == XIL_SHORT) {
                if(tablewarp_general_storage_16(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                    //  processing the next box.  Otherwise, return XIL_FAILURE
                    //  now. 
                ...
                if(tablewarp_general_storage_f32(td) == XIL_FAILURE) {
                    //
                    //  Mark this box as failed and if that succeeds, continue
                    //  processing the next box.  Otherwise, return XIL_FAILURE
                    //  now. 
...
static
XilStatus
tablewarp_pixel_sequential_16(TablewarpData td)
{
    ...
    unsigned int h_subsamples  = td.h_subsamples;
    ...
    //  source. 
    //
    float src_to_box_x = td.src_x_origin - src_box_x;
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned int i=dst_xstart; i<dst_xend;  i++) {
                ...
                //
                float fsx = i +
                    xili_convert_tablewarp_offset(*warp_pixel) + src_to_box_x;
                float fsy = j +
                    xili_convert_tablewarp_offset(*(warp_pixel+1)) + src_to_box_y;

                //
                //  Compute the integer and fracx portions.
                //
                int   isx   = XILI_GEOM_ROUNDDOWN(fsx);
                ...
                //
                //  Point to the correct location in the interpolation
                //  tables.
                //
                const float* h_ptr = h_data +
                    ((int)(fracx * h_subsamples)) * h_size;
                const float* v_ptr  = v_data +
                    ((int)(fracy * v_subsamples)) * v_size;

                //
                //  Compute interpolated pixel value.
                //
                Xil_unsigned8* pixel_addr =
                    src_data + sytable[isy] + sxtable[isx] -
                    src_scanline_stride*v_key - src_pixel_stride*h_key;
                Xil_unsigned8* dst_band   = dst_pixel;
                unsigned int   band_count = nbands;

                do {
                    XILI_INTERP_GENERAL_BYTE(pixel_addr,
                                             src_scanline_stride,
                                             src_pixel_stride,
                                             v_ptr,
                                             h_ptr,
...
static
XilStatus
tablewarp_pixel_sequential_f32(TablewarpData td)
{
    XilStorage*  src_storage   = td.src_storage;
    XilStorage*  warp_storage  = td.warp_storage;
    XilStorage*  dst_storage   = td.dst_storage;
...
static
XilStatus
tablewarp_general_storage_16(TablewarpData td)
{
    XilStorage*  src_storage   = td.src_storage;
    XilStorage*  warp_storage  = td.warp_storage;
    XilStorage*  dst_storage   = td.dst_storage;
    ...
    Xil_signed16*  warp_data_b1;
    unsigned int   warp_pixel_stride_b1;
    unsigned int   warp_scanline_stride_b1;
    warp_storage->getStorageInfo((unsigned int)1,
                                 &warp_pixel_stride_b1,
                                 &warp_scanline_stride_b1,
                                 NULL,
                                 (void**)&warp_data_b1);
    ...
    for(unsigned int b=0; b<nbands; b++) {
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            Xil_signed16*  warp_scanline_b1 = warp_data_b1 +
                y*warp_scanline_stride_b1 + x*warp_pixel_stride_b1;
            ...
            //  Scanlines in the destination.
            //
            for(float j=(float)dst_ystart; j<dst_yend; j++) {
                //
                //  Point to the first pixel of the scanline 
                //
                Xil_signed16*  warp_pixel_b0 = warp_scanline_b0;
                Xil_signed16*  warp_pixel_b1 = warp_scanline_b1;
                Xil_unsigned8* dst_pixel     = dst_scanline;

                for(float i=(float)dst_xstart; i<dst_xend;  i++) {
                    //
                    //  Calculate the source coordinates by reading the
                    //  displacement from the warp table and then moving the
                    //  values into source box space.
                    //
                    float fsx =
                        i + xili_convert_tablewarp_offset(*warp_pixel_b0) +
                        src_to_box_x;
                    float fsy =
                        j + xili_convert_tablewarp_offset(*warp_pixel_b1) +
                        src_to_box_y;
                    ...
                    warp_pixel_b1 += warp_pixel_stride_b1;
                ...
                warp_scanline_b1 += warp_scanline_stride_b1;
                dst_scanline += dst_scanline_stride;
            }
        }
...
static
XilStatus
tablewarp_general_storage_f32(TablewarpData td)
{
    XilStorage*  src_storage   = td.src_storage;
    XilStorage*  warp_storage  = td.warp_storage;
    XilStorage*  dst_storage   = td.dst_storage;
    ...
    for(unsigned int b=0; b<nbands; b++) {
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            Xil_float32*   warp_scanline_b1 = warp_data_b1 +
                y*warp_scanline_stride_b1 + x*warp_pixel_stride_b1;
            ...
            for(float j=(float)dst_ystart; j<dst_yend; j++) {
                ...
                Xil_float32*   warp_pixel_b1 = warp_scanline_b1;
                Xil_unsigned8* dst_pixel     = dst_scanline;

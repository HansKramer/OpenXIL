...
XilStatus
XilDeviceManagerComputeBYTE::Paint(XilOp*       op,
				   unsigned int   ,
				   XilRoi*      roi,
				   XilBoxList*  bl)
{
	   ...
	   (dest_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
		...
		while(paint_rl.getNext(&x, &y, &xsize, &ysize)) {
			...
			do {
				...
				*dest_band = _XILI_ROUND_U8(painted);
		...
		while(paint_rl.getNext(&x, &y, &xsize, &ysize)) {
			...
			do {
				...
				float painted =
                                    ((1.0F - *brush_pixel) *
                                     _XILI_B2F(*src1_pixel)) +
                                    (*brush_pixel * color[band]);
				*dest_pixel = _XILI_ROUND_U8(painted);

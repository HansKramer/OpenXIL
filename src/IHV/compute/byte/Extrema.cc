...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Extrema.cc	1.20\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeBYTE::Extrema(XilOp*       op,
                                      unsigned     op_count,
                                      XilRoi*      roi,
                                      XilBoxList*  bl)
{
...
#if !defined(HPUX)
    if(nbands < 256) {
        max  = (Xil_unsigned8*)alloca(nbands*sizeof(Xil_unsigned8));
        min  = (Xil_unsigned8*)alloca(nbands*sizeof(Xil_unsigned8));
        used_new = FALSE;
    } else {
#endif // !HPUX
    ...
    while(ci.hasMoreInfo()) {
        COMPUTE_GENERAL_1S_W_BAND(Xil_unsigned8,

...
//------------------------------------------------------------------------
//
//  File:	TablewarpBilinear.cc
//  Project:	XIL
//  Revision:	1.21
//  Last Mod:	10:11:06, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpBilinear.cc	1.21\t00/03/10  "
...
//
static XilStatus tablewarp_bilinear(XilBoxList*  bl, TablewarpData td);
...
XilStatus
XilDeviceManagerComputeBYTE::TablewarpBilinear(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    status = tablewarp_bilinear(bl, td);
...
static XilStatus
tablewarp_bilinear(XilBoxList*  bl,
		  TablewarpData td)
{
    XilOp*    op   = td.op;
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(float j=(float)dst_ystart; j<dst_yend; j++) {
                ...
                for(float i=(float)dst_xstart; i<dst_xend;  i++) {
                    ...
                    *dst_pixel =
                        xili_interp_bilinear(src_data +
                                             sytable[isy] + sxtable[isx],
                                             src_pixel_stride,
                                             src_scanline_stride,
                                             fracx,
                                             fracy);

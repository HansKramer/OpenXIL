...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Convolve.cc	1.26\t00/03/10  "
...
static
XilStatus
xili_convolve_square_opt_1(Xil_unsigned8* dst_data,
...
static
XilStatus
xili_convolve_3x3_opt_1(Xil_unsigned8* dst_data,
                        unsigned int   dst_sstride,
                        unsigned int   dst_pstride,
                        Xil_unsigned8* src_data,
                        unsigned int   src_sstride,
...
XilStatus
XilDeviceManagerComputeBYTE::Convolve(XilOp*       op,
				      unsigned     ,
				      XilRoi*      roi,
				      XilBoxList*  bl)
{
	...
	if(tag != XIL_AREA_CENTER) {
		...
		switch(tag) {
		  case XIL_AREA_TOP_LEFT_CORNER:
			...
			for(j = 0; j < ysize; j++) {
				...
				for(kh = 0; kh < keyy - ptrY - j; kh++) {
					...
					fsum += (_XILI_B2F(*sptr) * *(kptr + kh * kwidth + kw));
				    }
		  ...
		  case XIL_AREA_TOP_EDGE:
			...
			if(kulX < 0) kulX = 0;
			if(kulY < 0) kulY = 0;
			Xil_unsigned8* kernel_start = src1_scanline +
			    ((kulX - ptrX) * src1_pixel_stride) +
			    ((kulY - ptrY) * src1_scanline_stride);
			...
			for(j = 0; j < ysize; j++) {
				...
				Xil_unsigned8* sptr;
				Xil_unsigned8* corner = kernel_start + (i * src1_pixel_stride);
		  ...
		  case XIL_AREA_TOP_RIGHT_CORNER:
			...
			for(j = 0; j < ysize; j++) {
				...
				Xil_unsigned8* corner = top_right;
				Xil_unsigned8* sptr;
				int kh;
				int kw;
		  ...
		  case XIL_AREA_LEFT_EDGE:
			...
			for(j = 0; j < ysize; j++) {
				...
				Xil_unsigned8* sptr = kernel_start;
				Xil_unsigned8* sptr_save = sptr;
				int kh;
				int kw;
		  ...
		  case XIL_AREA_BOTTOM_LEFT_CORNER:
			...
			for(j = 0; j < ysize; j++) {
				...
				Xil_unsigned8* corner = lower_left;
				Xil_unsigned8* sptr = corner;
				Xil_unsigned8* sptr_save = corner;
				int kh;
				int kw;
		  ...
		  case XIL_AREA_BOTTOM_EDGE:
			...
			for(j = 0; j < ysize; j++) {
			    ...
			    Xil_unsigned8* src1_pixel = src1_scanline;
			    Xil_unsigned8* dest_pixel = dest_scanline;
			    Xil_unsigned8* lower_left = kernel_start;
				...
				Xil_unsigned8* sptr = lower_left;
				Xil_unsigned8* sptr_save = lower_left;
				int kh;
				int kw;
		  ...
		  case XIL_AREA_BOTTOM_RIGHT_CORNER:
			...
			for(j = 0; j < ysize; j++) {
				...
				Xil_unsigned8* corner = lower_right;
				Xil_unsigned8* sptr = corner;
				Xil_unsigned8* sptr_save = corner;
				int kh;
				int kw;
		...
		dest_storage.getStorageInfo(&dest_pixel_stride,
                ...
                if(nbands == 1) {
                    ...
                    while(rl.getNext(&x, &y, &xsize, &ysize)) {
                        ...
                        Xil_unsigned8* kernel_scanline  = src1_scanline -
                            (keyx * src1_pixel_stride) - (keyy * src1_scanline_stride);

                        do {
                            Xil_unsigned8* src1_pixel   = src1_scanline;
                            Xil_unsigned8* dest_pixel   = dest_scanline;
                            Xil_unsigned8* kernel_pixel = kernel_scanline;
                            unsigned int   pixel_count  = xsize;

                            do {
                                float  fsum                 = 0.0F;
                                float* kptr                 = kdata;
                                Xil_unsigned8* sptr_save    = kernel_pixel;
                                Xil_unsigned8* sptr         = sptr_save;
                                ...
                                *dest_pixel = _XILI_ROUND_U8(fsum);
				...
				for(unsigned int kh = kheight; kh != 0; kh--) {
				    for(unsigned int kw = kwidth; kw != 0; kw--) {
					fsum0 += _XILI_B2F(*sptr)     * *kptr;
					fsum1 += _XILI_B2F(*(sptr+1)) * *kptr;
					fsum2 += _XILI_B2F(*(sptr+2)) * *kptr;
				...
				*dest_pixel     = _XILI_ROUND_U8(fsum0);
				*(dest_pixel+1) = _XILI_ROUND_U8(fsum1);
				*(dest_pixel+2) = _XILI_ROUND_U8(fsum2);
                    ...
                    while(rl.getNext(&x, &y, &xsize, &ysize)) {
                        ...
                        do {
                            ...
                            do {
                                ...
                                Xil_unsigned8* kernel_band = kernel_pixel;
                                unsigned int   band_count  = nbands;

                                do {
                                    float  fsum                 = 0.0F;
                                    float* kptr                 = kdata;
                                    Xil_unsigned8* sptr_save    = kernel_band;
                                    Xil_unsigned8* sptr         = sptr_save;
                                    unsigned int   src1_sstride = src1_scanline_stride;
                                    unsigned int   src1_pstride = src1_pixel_stride;
		...
		while(rl.getNext(&x, &y, &xsize, &ysize)) {
			...
			do {
				...
				float fsum = 0.0;
				float* kptr = kdata;
				Xil_unsigned8* sptr = kernel_pixel;
				Xil_unsigned8* sptr_save  = sptr;
				
				for(unsigned int kh = 0; kh < kheight; kh++) {
				    for(unsigned int kw=0; kw < kwidth; kw++) {
					fsum += _XILI_B2F(*sptr) * *kptr;
      ...
      float*         results)
{
    ...
    float fpixel1= pixels[0];
    float fpixel2= pixels[pstride];
    float fpixel3= pixels[pstride * 2];
    float fpixel4= pixels[pstride * 3];
    float k;
    float results1;
    float results2;
    float results3;
...
xili_convolve_square_opt_1(Xil_unsigned8* dst_data,
                           ...
                           int            ksize)
{
    float* results;              // a pointer to the beginning of the result buffer
    float* resultsend;           // a pointer to the end of the result buffer
    float* resultptr;            // a ptr to the area to add the pixel contribution
    float* finishedptr;          // points to the results row which has been finished
    ...
    //
    results = new float[roi_xsize*ksize];
    if(results == NULL) {
        return XIL_FAILURE;
    }

    resultsend = results + roi_xsize*ksize;
    xili_memset(results, 0, roi_xsize*ksize*sizeof(float));

    //
    //  Create a buffer which has two copies of the kernel so that a wrapped
    //  version of it with any phase can be used
    //
    float* localkernel = new float[ksize_sqrd<<1];
    if(localkernel == NULL) {
        delete [] results;
        return XIL_FAILURE;
    }

    for(unsigned int i=0; i<ksize_sqrd; i++) {
        int j = ksize_sqrd - i - 1;
        localkernel[j]              = kdata[i];
        localkernel[ksize_sqrd + j] = kdata[i];
    }
    ...
    //  of madd()
    //
    float*         kernelptr   = localkernel+ksize_sqrd-1;
    ...
    Xil_unsigned8* srcptr;
    Xil_unsigned8* dstptr      = dst_data;
    ...
    for(y=1; y < ksize; y++) {
        ...
        int x;
        for(x=1; x<ksize; x++) {
            madd(*srcptr, kernelptr--, ksize, roi_xsize, x, y, resultptr);
            srcptr += src_pstride;
        }

        for(x=0; x < (roi_xsize-ksize+1); x++) {
            madd(*srcptr, kernelptr, ksize, roi_xsize, ksize, y, resultptr++);
            srcptr += src_pstride;
        }
    ...
    for(y=roi_ysize; y != 0; y--) {
        ...
        //
        for(x=1; x < ksize; x++) {
            madd(*srcptr, kernelptr--, ksize, roi_xsize, x, ksize, resultptr);
            srcptr += src_pstride;
        }
        ...
        for(x=remaining; x != 0; x--) {
            madd(*srcptr, kernelptr, ksize, roi_xsize, ksize, ksize, resultptr++);
            srcptr += src_pstride;
        }
                        ...
                        float*         kdata)
{
    ...
    for(y=ycount; y!=0; y--) {
        ...
        //  Pass 2
        //
        inA= _XILI_B2F(src_data[0]);

        temp3 = inA * k33;
        r1a  = temp3;

        inA= _XILI_B2F(src_data[srow1]);
        ...
        while(1) {
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2a);
            ...
            dst_data[drow2]= _XILI_ROUND_U8(r3a);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4a);
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2b);
            ...
            dst_data[drow2]= _XILI_ROUND_U8(r3b);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4b);
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2c);
            ...
            dst_data[drow2]= _XILI_ROUND_U8(r3c);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4c);
...
static
XilStatus
xili_convolve_3x3_opt_3(Xil_unsigned8* dst_data,
                        unsigned int   dst_sstride,
                        unsigned int   dst_pstride,
                        Xil_unsigned8* src_data,
                        unsigned int   src_sstride,
                        ...
                        float*         kdata)
{
    ...
    for(y=ycount; y!=0; y--) {
        ...
        inA = _XILI_B2F(src_data[1]);

        temp3 = inA * k33;
        r1a1  = temp3;

        inA = _XILI_B2F(src_data[2]);
        ...
        inA = _XILI_B2F(src_data[srow1+1]);
        ...
        inA = _XILI_B2F(src_data[srow1+2]);
        ...
        inA = _XILI_B2F(src_data[srow2 + 1]);
        ...
        inA = _XILI_B2F(src_data[srow2 + 2]);
        ...
        inA= _XILI_B2F(src_data[srow3 + 1]); 
        ...
        inA= _XILI_B2F(src_data[srow3 + 2]);
        ...
        inA= _XILI_B2F(src_data[srow4+1]);
        ...
        inA= _XILI_B2F(src_data[srow4+2]);
        ...
        inA= _XILI_B2F(src_data[srow5+1]);
    
        temp3 = inA * k31;   
        r4a1 += temp3;   

        inA= _XILI_B2F(src_data[srow5+2]);
        ...
        while(1) {
            ...
            dst_data[0]= _XILI_ROUND_U8(r1a0);
            ...
            dst_data[1]= _XILI_ROUND_U8(r1a1);
            ...
            dst_data[2]= _XILI_ROUND_U8(r1a2);
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2a0);
            ...
            dst_data[drow1+1]= _XILI_ROUND_U8(r2a1);
            ...
            dst_data[drow1+2]= _XILI_ROUND_U8(r2a2);
            ...
            dst_data[drow2] = _XILI_ROUND_U8(r3a0);
            ...
            dst_data[drow2 + 1] = _XILI_ROUND_U8(r3a1);
            ...
            dst_data[drow2 + 2] = _XILI_ROUND_U8(r3a2);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4a0);
            ...
            dst_data[drow3 + 1]= _XILI_ROUND_U8(r4a1);
            ...
            dst_data[drow3 + 2]= _XILI_ROUND_U8(r4a2);
            ...
            dst_data[0]= _XILI_ROUND_U8(r1b0);
            ...
            dst_data[1]= _XILI_ROUND_U8(r1b1);
            ...
            dst_data[2]= _XILI_ROUND_U8(r1b2);
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2b0);
            ...
            dst_data[drow1 + 1]= _XILI_ROUND_U8(r2b1);
            ...
            dst_data[drow1 + 2]= _XILI_ROUND_U8(r2b2);
            ...
            dst_data[drow2]= _XILI_ROUND_U8(r3b0);
            ...
            dst_data[drow2 + 1]= _XILI_ROUND_U8(r3b1);
            ...
            dst_data[drow2 + 2]= _XILI_ROUND_U8(r3b2);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4b0);
            ...
            dst_data[drow3 + 1]= _XILI_ROUND_U8(r4b1);
            ...
            dst_data[drow3 + 2]= _XILI_ROUND_U8(r4b2);
            ...
            dst_data[0]= _XILI_ROUND_U8(r1c0);
            ...
            dst_data[1]= _XILI_ROUND_U8(r1c1);
            ...
            dst_data[2]= _XILI_ROUND_U8(r1c2);
            ...
            dst_data[drow1]= _XILI_ROUND_U8(r2c0);
            ...
            dst_data[drow1 + 1]= _XILI_ROUND_U8(r2c1);
            ...
            dst_data[drow1 + 2]= _XILI_ROUND_U8(r2c2);
            ...
            dst_data[drow2]= _XILI_ROUND_U8(r3c0);
            ...
            dst_data[drow2 + 1]= _XILI_ROUND_U8(r3c1);
            ...
            dst_data[drow2 + 2]= _XILI_ROUND_U8(r3c2);
            ...
            dst_data[drow3]= _XILI_ROUND_U8(r4c0);
            ...
            dst_data[drow3 + 1]= _XILI_ROUND_U8(r4c1);
            ...
            dst_data[drow3 + 2]= _XILI_ROUND_U8(r4c2);

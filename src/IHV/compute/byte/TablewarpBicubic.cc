...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpBicubic.cc	1.21\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeBYTE::TablewarpBicubic(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned int i=dst_xstart; i<dst_xend;  i++) {
                ...
                do {      
                    XILI_INTERP_BICUBIC_BYTE(src_band,
                                             src_scanline_stride,
                                             src_pixel_stride,
                                             fracx,
                                             fracy,
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(float j=(float)dst_ystart; j<dst_yend; j++) {
                ...
                for(float i=(float)dst_xstart; i<dst_xend;  i++) {
                    ...
                    XILI_INTERP_BICUBIC_BYTE(src_pixel,
                                             src_scanline_stride,
                                             src_pixel_stride,
                                             fracx,
                                             fracy,

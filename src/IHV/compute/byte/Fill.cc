...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Fill.cc	1.8\t00/03/10  "
...
struct bfill8_stack_block {
    bfill8_stack_block* prev;
    bfill8_stack_block* next;
    bfill8_stack_frame  frame[XILI_FILL_MAX_STACK_FRAMES];
};

struct bfill8_stack_ctrl {
    bfill8_stack_block* curr_block;
    int                 curr_idx;
};
...
struct bfill8_data {
    ...
    Xil_unsigned8*     bound;            // boundary pixel array
    unsigned int       nbands;
    ...
    bfill8_stack_ctrl* stack_ctrl;       // points to stack ctrl block
};
...
//
static int BoundaryFill8(int            x,
                         int            y,
                         Xil_unsigned8* src_pixel,
                         Xil_unsigned8* fill_scan,
                         bfill8_data*   fill_ptr);

static int BFill8Push(bfill8_stack_ctrl*  ctrl_ptr,
                      bfill8_stack_frame* local_frame_ptr);

static int BFill8Pop(bfill8_stack_ctrl*  ctrl_ptr,
                     bfill8_stack_frame* local_frame_ptr);

inline Xil_boolean BFill8PixelBoundary(Xil_unsigned8* pixel,
                                       Xil_unsigned8* bound,
                                       unsigned int   nbands);

static void BFill8Shadow(bfill8_stack_frame* frame,
                         bfill8_data*        fill_ptr,
                         int                 lx,
                         Xil_unsigned8*      pixel,
                         int                 rx);

static int  BoundaryFill8G(int            x,
                           int            y,
...
static Xil_boolean BFill8PixelBoundaryG(XilStorage*    storage,
                                        unsigned int   x,
                                        unsigned int   y,
                                        Xil_unsigned8* bound,
                                        unsigned int   nbands,
...
//
XilStatus
XilDeviceManagerComputeBYTE::Fill(XilOp*       op,
                                  unsigned     ,
                                  XilRoi*      roi,
                                  XilBoxList*  bl)
{
    ...
    while(bl->getNext(&src_box, &dst_box)) {
        ...
        if((src_storage.isType(XIL_PIXEL_SEQUENTIAL)) &&
           (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
            ...
            //
            Xil_unsigned8* src_pixel = src_data +
            ...
            //
            bfill8_stack_ctrl* ctrl = new bfill8_stack_ctrl;
            if(ctrl == NULL) {
                delete [] fill_base_addr;
                XIL_ERROR(err_state, XIL_ERROR_RESOURCE, "di-1", TRUE);
                return XIL_FAILURE;
            }
            bfill8_stack_block* block = new bfill8_stack_block;
            if(block == NULL) {
                delete ctrl;
                delete [] fill_base_addr;
                XIL_ERROR(err_state, XIL_ERROR_RESOURCE, "di-1", TRUE);
            ...
            //  coordinates here.
            //
            if(BoundaryFill8(x_seed, y_seed, src_pixel, fill_scanline,
            ...
            //
            //  NOTE:  The src_pixel in bfill8_stack_frame and related slots
            //         in bfill8_data are not used for this case.
            //
            ...
            //
            if(BoundaryFill8G(x_seed, y_seed, &src_storage, fill_scanline,
                              &fill_data, srcbox_x, srcbox_y) == XIL_FAILURE) {
                //
                //  Failed during fill operation, attempt to clean up and
                //  return.  Must delete chain of alloc blocks following
                //  the next ptr
...
inline
Xil_boolean
BFill8PixelBoundary(Xil_unsigned8* pixel,
                    Xil_unsigned8* bound,
                    unsigned int   nbands)
{
              ...
              bfill8_data*   fill_ptr)
{
    ...
    if((y <= fill_ptr->ymax) && (y >= 0)) {
        ...
        //
        while((lx >= 0) && !(BFill8PixelBoundary(left_src_pixel,
                                                 fill_ptr->bound,
                                                 fill_ptr->nbands)) ) {
            //
            //  Set pixel mask
    ...
    if(BFill8Push(fill_ptr->stack_ctrl, &curr_frame) == XIL_FAILURE) {
        return XIL_FAILURE;
    }
    ...
    while(!(BFILL_STACK_EMPTY(fill_ptr->stack_ctrl))) {
        ...
        //
        if(BFill8Pop(fill_ptr->stack_ctrl, &curr_frame) == XIL_FAILURE) {
           return XIL_FAILURE;
        }
        ...
        if((y <= fill_ptr->ymax) && (y >= 0)) {
            ...
            while(x <= fill_ptr->xmax) {
                ...
                if(inside) {
                    if(!(BFill8PixelBoundary(src_pixel,
                                             fill_ptr->bound,
                                             fill_ptr->nbands)) &&
                       (!(BFILL_MASK_ON(fill_scan, x)))) {
                        //
                        ...
                        //  No longer inside
                        //
                        BFill8Shadow(&curr_frame, fill_ptr,
            ...
            if(inside) {
                ...
                //
                BFill8Shadow(&curr_frame, fill_ptr, lx, left_src_pixel, (x-1));
            }
        }
    }
...
static
int
BFill8Push(bfill8_stack_ctrl*  ctrl_ptr, 
           bfill8_stack_frame* local_frame_ptr)
{
    int        idx;
...
static
int
BFill8Pop(bfill8_stack_ctrl*  ctrl_ptr, 
          bfill8_stack_frame* local_frame_ptr)
{
    int        idx;
...
static
void
BFill8Shadow(bfill8_stack_frame* frame,
             bfill8_data*        fill_ptr,
             int                 lx,
             Xil_unsigned8*      pixel,
             int                 rx)
                                
{
    ...
    BFill8Push(fill_ptr->stack_ctrl, &new_frame);
               ...
               int            box_y)
{
    ...
    if((y <= fill_ptr->ymax) && (y >= 0)) {
        ...
        //  while INSIDE go left
        //
        while((lx >= 0) && !(BFill8PixelBoundaryG(src_storage, lx, y,
                                                  fill_ptr->bound,
                                                  fill_ptr->nbands,
                                                  box_x, box_y)) ) {
            //
        ...
        //
        while((rx <= fill_ptr->xmax) &&
              !(BFill8PixelBoundaryG(src_storage, rx, y,
                                     fill_ptr->bound,
                                     fill_ptr->nbands,
                                     box_x, box_y))) {
            //
    ...
    while(!(BFILL_STACK_EMPTY(fill_ptr->stack_ctrl))) {
        ...
        if((y <= fill_ptr->ymax) && (y >= 0)) {
            ...
            //
            inside =
                (!(BFill8PixelBoundaryG(src_storage, x, y,
                                        fill_ptr->bound,
                                        fill_ptr->nbands,
                                        box_x, box_y))) &&
                (!(BFILL_MASK_ON(fill_scan, x)));
            ...
            while(x <= fill_ptr->xmax) {
                ...
                if(inside) {
                    if(!(BFill8PixelBoundaryG(src_storage, x, y,
                                              fill_ptr->bound,
                                              fill_ptr->nbands,
                                              box_x, box_y)) &&
                       (!(BFILL_MASK_ON(fill_scan, x)))) {
                        ...
                        //  No longer inside
                        //
                        BFill8Shadow(&curr_frame, fill_ptr, lx, NULL, (x-1));
                        inside = FALSE;
                    }
                } else {
                    if(x > curr_frame.rx) {
...
static
Xil_boolean
BFill8PixelBoundaryG(XilStorage*    storage,
                     unsigned int   x,
                     unsigned int   y,
                     Xil_unsigned8* bound,
                     unsigned int   nbands,

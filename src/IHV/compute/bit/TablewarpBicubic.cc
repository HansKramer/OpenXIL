...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident        "@(#)TablewarpBicubic.cc	1.18\t00/03/10  "
...
static XilStatus tablewarp_general_storage_16(TablewarpData tablewarp_data);
static XilStatus tablewarp_general_storage_f32(TablewarpData tablewarp_data);


XilStatus
XilDeviceManagerComputeBIT::TablewarpBicubic(XilOp*       op,
                                             unsigned     ,
                                             XilRoi*      roi,
                                             XilBoxList*  bl)
{
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
            ...
            for(float j=dst_ystart; j<dst_yend; j++) {
                ...
                for(float i=dst_xstart; i<dst_xend;  i++) {
                    ...
                    //  source.
                    //
		    Xil_unsigned8* src_scanline   = src_data + sytable[isy] -
                                                    src_scanline_stride;
                    int   src_bit                 = isx + src_offset - 1;
                    if(src_bit < 0) {
                        unsigned int factor = -(src_bit + 1)/BIT_NUM_BITS + 1;
                        src_scanline -= factor;
                        src_bit += BIT_NUM_BITS*factor;
                    }

                    //
                    //  Do bicubic interpolation
                    //
                    XILI_INTERP_BICUBIC_BIT(src_scanline,
                                            src_scanline_stride,

...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident        "@(#)TablewarpBilinear.cc	1.15\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeBIT::TablewarpBilinear(XilOp*       op,
                                              unsigned     ,
                                              XilRoi*      roi,
                                              XilBoxList*  bl)
{
    ...
    for(unsigned int b=0; b<nbands; b++) {
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
		    ...
		    Xil_unsigned8* src_scanline   = src_data + sytable[isy];
                    unsigned int   src_bit        = isx + src_offset;
                    unsigned int   dst_bit        =
                        (int)i - dst_xstart + dst_offset;

                    xili_interp_bilinear(src_scanline,
                                         src_scanline_stride,
                                         src_bit,
                                         fracx,
                                         fracy,

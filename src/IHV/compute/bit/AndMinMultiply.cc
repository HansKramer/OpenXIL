...
//------------------------------------------------------------------------
//
//  File:	AndMinMultiply.cc
//  Project:	XIL
//  Revision:	1.7
//  Last Mod:	10:09:15, 03/10/00
//
//  Description:
//	Same functionality for xil_and(), xil_min(), xil_multiply()
//	
//	
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)AndMinMultiply.cc	1.7\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeBIT::AndMinMultiply(XilOp*       op,
                                           unsigned     op_count,
                                           XilRoi*      roi,
                                           XilBoxList*  bl)
{
	...
	for(int b=0; b<nbands; b++) { // each band
	    ...
	    int src2_scanline_stride = ci.getSrc2ScanlineStride(b);
	    int dest_scanline_stride = ci.getDestScanlineStride(b);

	    for(int y=ci.ysize; y>0; y--) { // each scanline
	   
		xili_bit_and(src1_scanline, src2_scanline, dest_scanline,
			ci.xsize, src1_offset, src2_offset, dest_offset); 
...
void
XilDeviceManagerComputeBIT::xili_bit_and(Xil_unsigned8* src1,
					 Xil_unsigned8* src2,
					 Xil_unsigned8* dest,
					 int	        width,
					 int            src1_offset,

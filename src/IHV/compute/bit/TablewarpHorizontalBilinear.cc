...
//------------------------------------------------------------------------
//
//  File:	TablewarpHorizontalBilinear.cc
//  Project:	XIL
//  Revision:	1.16
//  Last Mod:	10:10:03, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident        "@(#)TablewarpHorizontalBilinear.cc	1.16\t00/03/10  "
...
static XilStatus tablewarp_h_bilinear(XilBoxList*  bl, TablewarpData td);
...
XilStatus
XilDeviceManagerComputeBIT::TablewarpHorizontalBilinear(XilOp*       op,
                                                        unsigned     ,
                                                        XilRoi*      roi,
                                                        XilBoxList*  bl)
{
    ...
    status = tablewarp_h_bilinear(bl, td);
...
static
XilStatus
tablewarp_h_bilinear(XilBoxList*   bl,
                   TablewarpData td)
{
    XilOp*    op     = td.op;
    ...
    for(unsigned int b=0; b<nbands; b++) {
        ...
        while(rl.getNext(&x, &y, &xsize, &ysize)) {
		    ...
		    Xil_unsigned8* src_scanline   = src_data + sytable[isy];
                    ...
                    unsigned int   src_bit_1      = isx + src_offset + 1;
                    unsigned int   dst_bit        =
                        (int)i - dst_xstart + dst_offset;

                    float p00 = (float)XIL_BMAP_TST(src_scanline,
                                                    src_bit);
                    float p10 = (float)XIL_BMAP_TST(src_scanline,
                                                    src_bit_1);

		    //
		    //  Do linear interpolation -- fracy == 0.0F
		    //
                    float p;
		    XILI_GEOM_BLEND(p, p00, p10, fracx);

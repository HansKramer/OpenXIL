...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)CopyWithPlanemask.cc	1.5\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeBIT::CopyWithPlanemask(XilOp*       op,
                                 	      unsigned     op_count,
                                 	      XilRoi*      roi,
                                 	      XilBoxList*  bl)
{
	...
	for(int b=0; b<nbands; b++) {
		...
		unsigned int   src1_scanline_stride =
                    ci.getSrc1ScanlineStride(b); 
		unsigned int   dest_scanline_stride =
                    ci.getDestScanlineStride(b); 

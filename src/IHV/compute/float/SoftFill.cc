...
} sfillF_stack_frame_t; 

struct sfillF_stack_block {
    struct sfillF_stack_block	*prev;
    struct sfillF_stack_block	*next;
    sfillF_stack_frame_t	frame[XILI_FILL_MAX_STACK_FRAMES];
};
typedef struct sfillF_stack_block sfillF_stack_block_t;

struct sfillF_stack_ctrl {
    struct sfillF_stack_block	*curr_block;
    int				curr_idx;
};
typedef struct sfillF_stack_ctrl sfillF_stack_ctrl_t;
...
typedef struct {
    ...
    struct sfillF_stack_ctrl	*stack_ctrl;
} sfillF_data_t;
...
//
static int
SoftFill8(long x, long y, Xil_float32* src_pixel,
	  Xil_unsigned8* fill_scan, float* weight_pixel,
	  sfillF_data_t* fill_ptr, XilSystemState* err_state);

static int
SfillFPush(sfillF_stack_ctrl_t *ctrl_ptr, 
	   sfillF_stack_frame_t *local_frame_ptr);

static int
SfillFPop(sfillF_stack_ctrl_t *ctrl_ptr, 
	  sfillF_stack_frame_t *local_frame_ptr);

static Xil_boolean
SfillFPixelInside(Xil_float32* pixel,
		  float* weight_pixel,
		  Xil_float32* bg,
		  unsigned int nbands,
		  float* mtx,
...
static void
SfillFShadow(sfillF_stack_frame_t *frame,
	     sfillF_data_t	 *fill_ptr,
	     long		 lx,
	     Xil_float32	 *pixel,
	     float	 	 *weight_pixel,
	     long		 rx);
...
static Xil_boolean
SfillFPixelInsideG(XilStorage* storage,
		   float* weight_pixel,
		   Xil_float32* bg,
		   unsigned int nbands,
		   float* mtx,
...
//
XilStatus
XilDeviceManagerComputeFLOAT::SoftFill(XilOp*       op,
				      unsigned,
				      XilRoi*      roi,
				      XilBoxList*  bl)
{
    ...
    Xil_float32* last_bgcolor = &bgcolor[nbands*(num_bg-1)];
	...
	//
	   ...
	   (dest_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
	    ...
	    //
	    Xil_float32* src_pixel = src1_data +
	    ...
	    //
	    sfillF_stack_ctrl_t *ctrl = new sfillF_stack_ctrl_t;
	    if(ctrl == NULL) {
		delete [] weight_base_addr;
		delete [] fill_base_addr;
		delete [] mtx_invert;
	    ...
	    sfillF_stack_block_t *block = new sfillF_stack_block_t;
	    if(block == NULL) {
		delete ctrl;
		delete [] weight_base_addr;
		delete [] fill_base_addr;
		...
		return XIL_FAILURE;
            ...
            while(rl.getNext(&x, &y, &xsize, &ysize)) {
                Xil_float32* dst_scanline = dest_data +
                    y * dest_scanline_stride +
		    x * dest_pixel_stride;
                Xil_float32* src_scanline = src1_data +
		...
		return XIL_FAILURE;
			...
			Xil_float32* dst_pixel = dst_scanline;
			Xil_float32* src_pixel = src_scanline;
			weight_pixel = weight_scanline;
			//
			// Translate to image space
			//
...
SoftFill8(long x, long y, Xil_float32* src_pixel,
	  ...
	  sfillF_data_t* fill_ptr, XilSystemState* err_state)
{
    sfillF_stack_frame_t  curr_frame;
    long  lx;
    long rx;
	...
	while ( (lx >= 0) && SfillFPixelInside(left_src_pixel,
					       left_weight_pixel,
					       fill_ptr->bg,
					       fill_ptr->nbands,
					       fill_ptr->mtx,
	...
	while ((rx <= fill_ptr->xmax) && SfillFPixelInside(src_pixel,
							   weight_pixel,
							   fill_ptr->bg,
							   fill_ptr->nbands,
							   fill_ptr->mtx,
    ...
    if (SfillFPush(fill_ptr->stack_ctrl,&curr_frame)==XIL_FAILURE) {
	return(XIL_FAILURE);
    }
	...
	if (SfillFPop(fill_ptr->stack_ctrl,&curr_frame)==XIL_FAILURE) {
	   return(XIL_FAILURE);
	}
	...
	if ((y <= fill_ptr->ymax) && (y >= 0)) {
	    ...
	    inside = SfillFPixelInside(src_pixel, weight_pixel,
		...
		while ((x >= 0) && SfillFPixelInside(src_pixel,
						     weight_pixel,
						     fill_ptr->bg,
						     fill_ptr->nbands,
						     fill_ptr->mtx,
		...
		if (inside) {
		    if (SfillFPixelInside(src_pixel, weight_pixel,
					  fill_ptr->bg,
					  fill_ptr->nbands,
					  fill_ptr->mtx, fill_ptr->nbg,
					  fill_scan, x)) {
			...
			// no longer inside
			SfillFShadow(&curr_frame, fill_ptr, lx,
		...
		SfillFShadow(&curr_frame, fill_ptr, lx, left_src_pixel,
...
{
    sfillF_stack_frame_t	new_frame;
    long			newy;
    ...
    SfillFPush(fill_ptr->stack_ctrl, &new_frame);
	   ...
	   int box_x, int box_y)
{
	...
	left_weight_pixel = weight_pixel;
        ...
        // check seed pixel within region
	while ( (lx >= 0) && SfillFPixelInsideG(src_storage,
						left_weight_pixel,
						fill_ptr->bg,
						fill_ptr->nbands,
						fill_ptr->mtx,
	...
	while ((rx <= fill_ptr->xmax) && SfillFPixelInsideG(src_storage,
							    weight_pixel,
							    fill_ptr->bg,
							    fill_ptr->nbands,
							    fill_ptr->mtx,
	...
	if ((y <= fill_ptr->ymax) && (y >= 0)) {
	    ...
	    // here to check left
	    weight_pixel = curr_frame.weight_pixel;
	    inside = SfillFPixelInsideG(src_storage, weight_pixel,
					fill_ptr->bg, fill_ptr->nbands,
					fill_ptr->mtx, fill_ptr->nbg,
		...
		while ((x >= 0) && SfillFPixelInsideG(src_storage,
						      weight_pixel,
						      fill_ptr->bg,
						      fill_ptr->nbands,
						      fill_ptr->mtx,
		...
		if (inside) {
		    if (SfillFPixelInsideG(src_storage, weight_pixel,
					   fill_ptr->bg,
					   fill_ptr->nbands,
					   fill_ptr->mtx, fill_ptr->nbg,
					   fill_scan, x, y, box_x, box_y)) {
	    ...
	    if (inside) {
		// hit the edge of region while still inside scan
		SfillFShadow(&curr_frame, fill_ptr, lx, NULL,
			     left_weight_pixel, (x-1));
	    }
	}    // end if within y bounds
    }    // end while, done with fill loop, stack empty

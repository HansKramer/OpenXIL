...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)SetValue.cc	1.8\t00/03/10  "

#include "XilDeviceManagerComputeFLOAT.hh"
#include "XiliUtils.hh"

XilStatus
XilDeviceManagerComputeFLOAT::SetValue(XilOp*       op,
                                       unsigned     ,
                                       XilRoi*      roi,
                                       XilBoxList*  bl)
{
        ...
        if(dst_storage.isType(XIL_PIXEL_SEQUENTIAL)) {
            ...
            while(rl.getNext(&x, &y, &xsize, &ysize)) {
                ...
                if(dst_pixel_stride == nbands) {
                    Xil_float32* first_scanline;
                    Xil_float32* tmp_scanline;
                    Xil_float32* next_scanline;
                    unsigned int scanline_len = xsize*nbands*sizeof(Xil_float32);
            ...
            for(unsigned int b=0; b<nbands; b++) {
                ...
                while(rl.getNext(&x, &y, &xsize, &ysize)) {
                    ...
                    if(dst_pixel_stride == 1) {
                        ...
                        Xil_float32* tmp_scanline;
                        Xil_float32* next_scanline;
                        unsigned int scanline_len = xsize*sizeof(Xil_float32);

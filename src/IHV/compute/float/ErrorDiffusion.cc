...
static XilStatus
default_Fto1_error_diffusion(XilOp* op,
			     XilRoi* roi,
			     XilBoxList* bl,
			     XilImage* src,
			     XilImage* dst,
...
static XilStatus
default_Fto8_error_diffusion(XilOp* op,
			     XilRoi* roi,
			     XilBoxList* bl,
			     XilImage* src,
			     XilImage* dst,
...
static XilStatus
default_Fto16_error_diffusion(XilOp* op,
			      XilRoi* roi,
			      XilBoxList* bl,
			      XilImage* src,
			      XilImage* dst,
...
static XilStatus
default_FtoF_error_diffusion(XilOp* op,
			     XilRoi* roi,
			     XilBoxList* bl,
			     XilImage* src,
			     XilImage* dst,
...
static XilStatus
xili_floyd_steinburg_1band_Fto1_error_diffusion(XilOp*           op,
                                                XilRoi*          roi,
                                                XilBoxList*      bl,
                                                XilImage*        src,
                                                XilImage*        dst,
...
//
// Perform a float to 1-bit ErrorDiffusion
//
XilStatus
XilDeviceManagerComputeFLOAT::ErrorDiffusion1(XilOp*       op,
					     unsigned,
					     XilRoi*      roi,
					     XilBoxList*  bl)
{
    ...
    if(src->getNumBands() == 1 && xili_is_floyd_steinberg_kernel(dist)) {
        stat = xili_floyd_steinburg_1band_Fto1_error_diffusion(
        ...
        //
        if(stat == XIL_FAILURE) {
            stat = default_Fto1_error_diffusion(
			     ...
			     XilSystemState* err_state)
{
	...
	//
        ...
        //  XIL_PIXEL_SEQUENTIAL.  If so, implement a loop optimized
	    ...
	    Xil_float32* src_scanline = src_data -
...
//
// Perform a float to 8-bit ErrorDiffusion
//
XilStatus
XilDeviceManagerComputeFLOAT::ErrorDiffusion8(XilOp*       op,
					     unsigned,
					     XilRoi*      roi,
					     XilBoxList*  bl)
{
	...
	case 1:
    		...
    		stat = FS_band1_Fto8(src, dst, cmap);

    	    else
    		stat = default_Fto8_error_diffusion(src, dst, cmap, dist);
	    break;
...
#else
    stat = default_Fto8_error_diffusion(op, roi, bl, src, dst, cmap, dist,
					 err_state);
#endif    
    return stat;
}
			      ...
			      XilSystemState* err_state)
{
	...
	//
	   ...
	   (dst_storage.isType(XIL_PIXEL_SEQUENTIAL))) {
	    ...
	    //
	    // Set up info to copy data from tmp to dst taking the ROI into
	    // account.  dst will always be 1-banded so we can just get the
	    // storage info once and treat it as a pixel-sequential image.
	    //
	    unsigned int dst_scanline_stride;
	    unsigned int dst_pixel_stride;
	    Xil_unsigned8* dst_data;
...
//
// Perform a float to 16-bit ErrorDiffusion
//
XilStatus
XilDeviceManagerComputeFLOAT::ErrorDiffusion16(XilOp*       op,
					      unsigned int,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
...
#else
    stat = default_Fto16_error_diffusion(op, roi, bl, src, dst, cmap,
					  dist, err_state);
#endif
    return stat;
}
...
#define FS_FTO1_1BAND_ERR_DIST(err)          \
{                                            \
    float err1 = err * 0.0625F;              \
    float err4 = err * 0.2500F;              \
    float err3 = err4 - err1;                \
    src_val = curr_pix[1] + (err4 + err3);   \
    curr_pix[-1] += err3;                    \
    curr_pix[0] += (err4 + err1);            \
    pSrc += src_pixel_stride;                \
    curr_pix++;                              \
    curr_pix[0] = err1 + *pSrc;   \
}

//////////////////////////////////////////////////////////////////////////
//
//
// Optimized case for single-band float to 1 bit error diffusion,
...
// pixels A, B, C, D.
//
// TODO: All distribution coefficients are in 16ths, so we could use 
//       scaled integer arithmetic. Currently, we use float math.
// 
// Coefficients are:
//      A: 7/16
//      B: 3/16
                                                ...
                                                XilSystemState*  err_state)
{
	...
	float* buf_mem          = new float[src_padded_width];
	if (buf_mem == NULL) {
	    delete [] tmp_base_addr;
	    XIL_ERROR(err_state, XIL_ERROR_RESOURCE, "di-1", TRUE);
	    return XIL_FAILURE;
	...
	float* buf = buf_mem + 1;
        ...
        //
        Xil_float32* cmap_data = (Xil_float32*)cmap->getData();

        //
        //  Since only 2 possible entries in colormap and dealing with 
        //  1-band in colormap, create sorted list and find mid point 
        //  so all that is needed is quick comparison.
        //
        float         mid_val = ((float)cmap_data[0] + (float)cmap_data[1]) * 0.5F;
        unsigned int lo_idx;
        unsigned int hi_idx;
        ...
        float lo_val = cmap_data[lo_idx];
        float hi_val = cmap_data[hi_idx];
        ...
        for(int y=0; y<end_scanline; y++) {
            ...
            //  Each execution of the FS_FTO1_1BAND_ERR_DIST macro

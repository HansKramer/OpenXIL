...
XilStatus
XilDeviceManagerComputeFLOAT::Threshold(XilOp*       op,
                                        unsigned     op_count,
                                        XilRoi*      roi,
                                        XilBoxList*  bl)
{
    ...
    while(ci.hasMoreInfo()) {
        Xil_float32* low  = op_low;
        Xil_float32* high = op_high;
        Xil_float32* map  = op_map;
...
XilStatus
XilDeviceManagerComputeFLOAT::ThresholdThreshold(XilOp*       op,
                                                 unsigned     op_count,
                                                 XilRoi*      roi,
                                                 XilBoxList*  bl)
{
    ...
    while(ci.hasMoreInfo()) {
        Xil_float32* low1  = op1_low;
        Xil_float32* high1 = op1_high;
        Xil_float32* map1  = op1_map;

        Xil_float32* low2  = op2_low;
        Xil_float32* high2 = op2_high;
        Xil_float32* map2  = op2_map;

	COMPUTE_GENERAL_1S_1D_W_BAND(Xil_float32, Xil_float32,

                                     Xil_float32 val = *src1;
                                     if(val > *high1) {
                                         if(val > *high2) {
                                             *dest = val;
                                         } else if(val < *low2) {
...
XilStatus
XilDeviceManagerComputeFLOAT::Threshold_1BAND(XilOp*       op,
                                             unsigned     op_count,
                                             XilRoi*      roi,
                                             XilBoxList*  bl)
{
    if(op->getDstImage(1)->getNumBands() != 1 ||
       sizeof(Xil_float32) != 2) {
        return XIL_FAILURE;
    }
    ...
    while(ci.hasMoreInfo()) {
        if(ci.destScanlineStride == ci.xsize &&
           ci.src1ScanlineStride == ci.xsize) {
            ...
            Xil_float32* src_data  = ci.src1Data;
            Xil_float32* dst_data  = ci.destData;
            Xil_float32* src       = src_data;
            Xil_float32* stop      = src_data + (ci.xsize*ci.ysize);
            ...
            while(src < stop) {
                if(status == 0) {
                    ...
                    int bytes = (current_i - status_i)<<2;
                    if(bytes < _XIL_FUNC_THRESH) {
                        Xil_float32* s = src_data+status_i;
                        Xil_float32* d = dst_data+status_i;

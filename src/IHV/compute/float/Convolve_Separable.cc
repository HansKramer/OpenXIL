...
static XilStatus
ConvolveSep(Xil_float32* dest_data,
...
XilStatus
XilDeviceManagerComputeFLOAT::Convolve_SeparablePreprocess(XilOp*        op,
                                                           unsigned      ,
                                                           XilRoi*       ,
                                                           void**        ,
                                                           unsigned int* )
...
XilStatus
XilDeviceManagerComputeFLOAT::Convolve_Separable(XilOp*        op,
                                                 unsigned        ,
                                                 XilRoi*      roi,
                                                 XilBoxList*  bl)
{
		  ...
		  case XIL_AREA_TOP_LEFT_CORNER:
			...
			for(j = 0; j < ysize; j++) {
				...
				for(kh = 0; kh < keyy - ptrY - j; kh++) {
				    ...
				    for(kw = 0; kw < keyx - ptrX - i - x; kw++) {
					fsum += ((*sptr) * *(kptr + kh * kwidth + kw));
				    }
				    
				    // for the upper right quadrant
				    for(kw = keyx - ptrX - i - x; kw < kwidth; kw++) {
					fsum += ((*sptr) * *(kptr + kh * kwidth + kw));
            ...
            int kheight)
{
    ...
    for( int b = 0; b < nbands; b++ ) {
        ...
        if( kwidth == 5 && kheight == 5 ) {
            ...
            //
            Xil_float32 *src = srcptr;
            Xil_float32 *dst = dstptr;
            Xil_float32 *tmp = srcptr;
            ...
            while( kh-- ) {
                for( j = 0; j < src1_xsize; j++ ) {
                    fsum  = (*src) * kx0;  src += src1_pixel_stride;
                    fsum += (*src) * kx1;  src += src1_pixel_stride;
                    fsum += (*src) * kx2;  src += src1_pixel_stride;
                    fsum += (*src) * kx3;  src += src1_pixel_stride;
                    fsum += (*src) * kx4;

...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpNearest.cc	1.12\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeFLOAT::TablewarpNearest(XilOp*       op,
					      unsigned     ,
					      XilRoi*      roi,
					      XilBoxList*  bl)
{
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned i=dst_xstart; i<dst_xend;  i++) {
                ...
                //
                Xil_float32*   src_band   = src_data + sytable[isy] + sxtable[isx];

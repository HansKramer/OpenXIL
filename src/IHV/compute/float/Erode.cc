...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Erode.cc	1.13\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeFLOAT::Erode(XilOp*       op,
                                   unsigned int    ,
                                   XilRoi*      roi,
                                   XilBoxList*  bl)
{
    ...
    Xil_float32 *mmin = new Xil_float32[nbands];
    if (mmin == NULL) {
      delete [] xdeltas;
      delete [] ydeltas;
      XIL_ERROR(dest->getSystemState(), XIL_ERROR_RESOURCE, "di-1", TRUE);
                ...
                for (int l_y = 0; l_y < ysize; l_y++) {
                  ...
                  for (int l_x = 0; l_x < xsize; l_x++) {
                    ...
                    if (nbands == 1) {
                      ...
                      for (int k1=0; k1<nbands; k1++)
                        mmin[k1] = XIL_MAXFLOAT;
                    } 
                    ...
                    for (int l_x = 0; l_x < xsize; l_x++) {
                      ...
                      for (int z=0; z<sel_size; z++) {
                        ...
                        switch (dest_box_type) {
                          ...
                          case XIL_AREA_TOP_EDGE:
                            if (ipdy + l_y >= 0) {
                              Xil_float32 *src_band = src_scanline +

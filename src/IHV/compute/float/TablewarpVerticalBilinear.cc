...
//------------------------------------------------------------------------
//
//  File:	TablewarpVerticalBilinear.cc
//  Project:	XIL
//  Revision:	1.16
//  Last Mod:	10:13:28, 03/10/00
//
//  Description:
//	
//	
...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)TablewarpVerticalBilinear.cc	1.16\t00/03/10  "
...
//
static XilStatus tablewarp_v_bilinear(XilBoxList*  bl, TablewarpData td);
...
XilStatus
XilDeviceManagerComputeFLOAT::TablewarpVerticalBilinear(XilOp*       op,
                                                       unsigned     ,
                                                       XilRoi*      roi,
                                                       XilBoxList*  bl)
{
    ...
    status = tablewarp_v_bilinear(bl, td);
...
static XilStatus
tablewarp_v_bilinear(XilBoxList*  bl,
		  TablewarpData td)
{
    XilOp*    op   = td.op;
    ...
    while(rl.getNext(&x, &y, &xsize, &ysize)) {
        ...
        for(unsigned j=dst_ystart; j<dst_yend; j++) {
            ...
            for(unsigned int i=dst_xstart; i<dst_xend;  i++) {
                ...
                do {      
                    *dst_band++ = xili_interp_linear(src_band++,
                                                     src_scanline_stride,
                                                     fracy);
                } while (--band_count);

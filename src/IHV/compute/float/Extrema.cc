...
//	COPYRIGHT
//------------------------------------------------------------------------
#pragma ident	"@(#)Extrema.cc	1.18\t00/03/10  "
...
XilStatus
XilDeviceManagerComputeFLOAT::Extrema(XilOp*       op,
                                      unsigned     op_count,
                                      XilRoi*      roi,
                                      XilBoxList*  bl)
{
    ...
    //
    Xil_float32* max = NULL;
    Xil_float32* min = NULL;
    Xil_boolean   used_new;

#if !defined(HPUX)
    if(nbands < 256) {
        max  = (Xil_float32*)alloca(nbands*sizeof(Xil_float32));
        min  = (Xil_float32*)alloca(nbands*sizeof(Xil_float32));
        used_new = FALSE;
    } else {
#endif // !HPUX
        max = new Xil_float32[nbands];
        if(max == NULL) {
            XIL_ERROR(ci.getSystemState(), XIL_ERROR_RESOURCE, "di-1", TRUE);
            return XIL_FAILURE;
        }
    
        min = new Xil_float32[nbands];
        if(min == NULL) {
            if(max)
                delete [] max;
            XIL_ERROR(ci.getSystemState(), XIL_ERROR_RESOURCE, "di-1", TRUE);
    ...
    while(ci.hasMoreInfo()) {
        COMPUTE_GENERAL_1S_W_BAND(Xil_float32,

...
*----------------------------------------------------------------------*/
#ifndef lint
static	char     sccsid[] = "@(#)window.c	1.7\t00/03/10  ";
#endif
            ...
            Xil_boolean    prefer_depth_24)
{
    ...
    } else if(XMatchVisualInfo(xdisplay, DefaultScreen(xdisplay), 8, PseudoColor, vinfo)) {
     ...
     */
    if(vinfo->class == GrayScale || vinfo->class == PseudoColor) {
